...
def test_explorer_locally():
    ...
    uel = loop.UniversalExperimentLoop(
        data=historical.data, sfd=loop.sfd.foundational_sfd.logreg_binary
    )

    print(
        "Keep waiting, there will now be three progress bars, once the last is done, you will see the streamlit url"
    )
