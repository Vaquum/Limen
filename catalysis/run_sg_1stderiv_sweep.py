#!/usr/bin/env python3
'''
SG Filter 1st Derivative Parameter Sweep - Quick Test

Tests momentum crossover strategy with 20 permutations.
'''
...
def run_1stderiv_sweep(n_permutations=20, months_back_data=20):
    '''
    Quick parameter sweep for 1st derivative momentum strategy.
    '''

    print('=' * 80)
    print('SG Filter 1st Derivative - Quick Parameter Sweep (20 permutations)')
    print('=' * 80)
    ...
    # Initialize UEL with 1st derivative SFM
    print(f'\n[2/3] Initializing UEL with tradeable_regressor_sg_1stderiv...')

    uel = loop.UniversalExperimentLoop(
        data=historical.data,
        single_file_model=loop.sfm.lightgbm.tradeable_regressor_sg_1stderiv
    )

    print(f'Testing 1st derivative momentum crossovers instead of inflections')

    # Run parameter sweep
    print(f'\n[3/3] Running UEL with {n_permutations} permutations...')
    print('This should be quick - ~6-7 minutes total\n')

    uel.run(
        experiment_name='sg_1stderiv_quick',
    ...
    for i in range(len(log)):
        ...
        trading_metrics.append({
            ...
            'min_momentum': extras['sg_config']['min_momentum_threshold'],
            'min_prediction': extras['sg_config']['min_prediction_level'],
            'min_spacing': extras['sg_config']['min_crossing_spacing'],
        })
    ...
    # Save results
    output_file = '/Users/beyondsyntax/Loop/catalysis/sg_1stderiv_results.csv'
    full_results.write_csv(output_file)
    print(f'\nResults saved to: {output_file}')

    # Display top performers
    print('\n' + '-' * 80)
    print('TOP 5 BY VALIDATION SHARPE RATIO')
    print('-' * 80)
    ...
    if len(valid_results) > 0:
        ...
        print(top_sharpe.select(['id', 'val_sharpe', 'val_total_return', 'val_win_rate',
                                ...
                                'min_momentum', 'min_prediction']))

        print('\n' + '-' * 80)
        print('TOP 5 BY TEST SET SHARPE RATIO')
        print('-' * 80)

        # Filter for configs with at least some test trades
        test_valid = full_results.filter(pl.col('test_num_trades') >= 5)
        if len(test_valid) > 0:
            top_test = test_valid.sort('test_sharpe', descending=True).head(5)
            print(top_test.select(['id', 'test_sharpe', 'test_total_return', 'test_win_rate',
                                   'test_num_trades', 'lookahead_minutes', 'sg_window',
                                   'min_momentum', 'min_prediction']))
        else:
            print("No configs with >= 5 test trades")

        # Best validation config test performance
        print('\n' + '=' * 80)
        ...
        print(f"Min Momentum: {best_row['min_momentum'][0]:.6f}")
        ...
        print("\nNO VALID RESULTS: All configurations produced < 10 trades")
...
if __name__ == '__main__':
    uel, results = run_1stderiv_sweep(n_permutations=20, months_back_data=20)
    print('\nDone!')
