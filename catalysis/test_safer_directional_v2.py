#!/usr/bin/env python3
"""
Test Safer Directional Signals v2

Uses 4-model approach:
- LONG model: P(LONG)
- SHORT model: P(SHORT)
- MOVEMENT model: P(MOVEMENT)
- BOTH model: P(LONG ∩ SHORT)
...
print('=' * 80)
print('SAFER DIRECTIONAL SIGNALS v2 (4-model approach)')
print('=' * 80)
...
# Train with UEL
print('\nTraining 4 directional conditional models...')
...
uel.run(
    experiment_name='safer_directional_v2',
    n_permutations=1,
    random_search=False
)
...
p_both_given_movement = probs['both_given_movement']
p_safe_long = probs['safe_long_given_movement']
p_safe_short = probs['safe_short_given_movement']
...
print('\nRaw Probabilities:')
print(f'  P(LONG):      mean={np.mean(p_long):.3f}, std={np.std(p_long):.3f}')
print(f'  P(SHORT):     mean={np.mean(p_short):.3f}, std={np.std(p_short):.3f}')
print(f'  P(MOVEMENT):  mean={np.mean(p_movement):.3f}, std={np.std(p_movement):.3f}')
print(f'  P(BOTH):      mean={np.mean(p_both):.3f}, std={np.std(p_both):.3f}')
...
print(f'  P(BOTH | movement):   mean={np.mean(p_both_given_movement):.3f}, std={np.std(p_both_given_movement):.3f}')

print('\nSafer Probabilities (excluding opposite direction):')
print(f'  P(LONG only | movement):  mean={np.mean(p_safe_long):.3f}, std={np.std(p_safe_long):.3f}')
print(f'  P(SHORT only | movement): mean={np.mean(p_safe_short):.3f}, std={np.std(p_safe_short):.3f}')
...
for task in ['long', 'short', 'movement', 'both']:
    dist = class_dists[task]
    total = sum(dist.values())
    pct_positive = dist.get(1, 0) / total * 100 if total > 0 else 0
    print(f'  {task.upper():8s}: {dist} ({pct_positive:.1f}% positive)')
...
print(f'  Safer (P(LONG|movement) - P(BOTH|movement) > 0.7):     {safer_long:5d} ({safer_long/len(p_long)*100:.1f}%)')
...
print(f'  Safer (P(SHORT|movement) - P(BOTH|movement) > 0.7):    {safer_short:5d} ({safer_short/len(p_short)*100:.1f}%)')
print(f'  Difference:                                              {safer_short - regular_short:+5d}')

# Analyze the difference - LONG focus
print('\n' + '=' * 80)
print('LONG SIGNAL OVERLAP ANALYSIS')
print('=' * 80)
...
print(f'  Only regular:                {long_only_regular:5d} (has BOTH risk - volatile)')
print(f'  Only safer:                  {long_only_safer:5d} (low BOTH, but lower LONG)')

# Show examples
print('\n' + '=' * 80)
print('EXAMPLE LONG SIGNALS')
print('=' * 80)

print('\nLONG - Both approaches (highest confidence):')
idx = np.where((p_long_given_movement > 0.7) & (p_safe_long > 0.7) & (p_movement > 0.3))[0]
if len(idx) > 0:
    for i in idx[:3]:
        print(f'  Sample {i}: P(LONG|mov)={p_long_given_movement[i]:.3f}, P(BOTH|mov)={p_both_given_movement[i]:.3f}, P(safe_long)={p_safe_long[i]:.3f}')
else:
    print('  None found')

print('\nLONG - Only in REGULAR (has BOTH/volatility risk):')
idx = np.where((p_long_given_movement > 0.7) & (p_safe_long <= 0.7) & (p_movement > 0.3))[0]
if len(idx) > 0:
    for i in idx[:3]:
        print(f'  Sample {i}: P(LONG|mov)={p_long_given_movement[i]:.3f}, P(BOTH|mov)={p_both_given_movement[i]:.3f}, P(safe_long)={p_safe_long[i]:.3f}')
...
print('\nLONG - Only in SAFER (low BOTH, moderate LONG):')
idx = np.where((p_long_given_movement <= 0.7) & (p_safe_long > 0.7) & (p_movement > 0.3))[0]
if len(idx) > 0:
    for i in idx[:3]:
        print(f'  Sample {i}: P(LONG|mov)={p_long_given_movement[i]:.3f}, P(BOTH|mov)={p_both_given_movement[i]:.3f}, P(safe_long)={p_safe_long[i]:.3f}')
...
# Analyze BOTH probability
print('\n' + '=' * 80)
print('BOTH (Whipsaw) ANALYSIS')
print('=' * 80)

high_both = np.sum(p_both_given_movement > 0.3)
very_high_both = np.sum(p_both_given_movement > 0.5)

print(f'\nSamples with P(BOTH|movement) > 0.3: {high_both} ({high_both/len(p_both)*100:.1f}%)')
print(f'Samples with P(BOTH|movement) > 0.5: {very_high_both} ({very_high_both/len(p_both)*100:.1f}%)')

print('\nExamples of high BOTH probability (whipsaw risk):')
idx = np.where(p_both_given_movement > 0.5)[0]
if len(idx) > 0:
    for i in idx[:5]:
        print(f'  Sample {i}: P(LONG|mov)={p_long_given_movement[i]:.3f}, P(SHORT|mov)={p_short_given_movement[i]:.3f}, P(BOTH|mov)={p_both_given_movement[i]:.3f}')
else:
    print('  None found')
...
results_df = pl.DataFrame({
    ...
    'p_both_given_movement': p_both_given_movement,
    'p_safe_long_given_movement': p_safe_long,
    'p_safe_short_given_movement': p_safe_short
})

output_file = '/Users/beyondsyntax/Loop/catalysis/safer_directional_v2_results.csv'
