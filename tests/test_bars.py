...
def validate_bars_output(result: pl.DataFrame, expected_aggregation: bool = True):
    ...
    if len(result) > 1:
        assert (
            result["datetime"].diff().drop_nulls().dt.total_nanoseconds() >= 0
        ).all()
