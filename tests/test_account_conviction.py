...
def validate_vector_consistency(
    account: Account, actions: list, prices: list, amounts: list
) -> dict:
    ...
    for i, (action, price, amount) in enumerate(zip(actions, prices, amounts)):
        if action == "buy":
            btc_bought = round(
                amount / price, BTC_PRECISION
            )  # Updated to match Account's 15-decimal precision
    ...
    assert abs(manual_long_btc - account_long) < btc_tolerance, (
        f"Manual long ({manual_long_btc}) != Account long ({account_long})"
    )
    assert abs(manual_short_btc - account_short) < btc_tolerance, (
        f"Manual short ({manual_short_btc}) != Account short ({account_short})"
    )
    assert abs(manual_usdt - account_usdt) < usdt_tolerance, (
        f"Manual USDT ({manual_usdt}) != Account USDT ({account_usdt})"
    )
    assert abs((manual_long_btc - manual_short_btc) - account_net) < btc_tolerance, (
        "Manual net != Account net"
    )

    assert abs((sum_bought - sum_sold) - account_long) < btc_tolerance, (
        "Sum-based long != Account long"
    )
    assert abs((sum_borrowed - sum_covered) - account_short) < btc_tolerance, (
        "Sum-based short != Account short"
    )
    assert abs((sum_credit - sum_debit) - account_usdt) < usdt_tolerance, (
        "Sum-based USDT != Account USDT"
    )

    # Vector 4: History integrity
    assert len(account.account["action"]) == len(actions) + 1, (
        f"Action count mismatch: {len(account.account['action'])} vs {len(actions) + 1}"
    )
    assert len(set(len(v) for v in account.account.values())) == 1, (
        "Inconsistent vector lengths"
    )

    # Vector 5: Invariant checks
    assert abs(account.account["total_btc"][-1] - account_long) < btc_tolerance, (
        "total_btc != long_position"
    )
    assert abs(account.net_position - (account_long - account_short)) < btc_tolerance, (
        "net_position calculation error"
    )
...
def test_deterministic_sequence() -> None:
    ...
    tolerance = 1e-7
    assert abs(account.long_position - expected_long) < tolerance, (
        "Deterministic long mismatch"
    )
    assert abs(account.short_position - expected_short) < tolerance, (
        "Deterministic short mismatch"
    )
    assert abs(account.account["total_usdt"][-1] - expected_usdt) < tolerance, (
        "Deterministic USDT mismatch"
    )
