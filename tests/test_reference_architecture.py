...
def _make_regression_data(n=200, with_val=True, with_price=True):
    np.random.seed(42)
    n_features = 5
...
def _make_binary_data(n=200, with_val=True, with_price=True):
    np.random.seed(42)
    n_features = 5
    x = np.random.randn(n, n_features)
    y = (x[:, 0] > 0).astype(int)
...
def test_xgboost_evaluate_returns_all_metric_types():
    data = _make_regression_data(with_price=True)
...
def test_logreg_train_evaluate_end_to_end():
    data = _make_binary_data(with_price=False)
...
def test_train_with_validation_data():
    data = _make_regression_data(with_val=True)
    model = XGBoostRegressor().train(data, learning_rate=0.1, n_estimators=10, random_state=42)
    assert model.model is not None
...
def test_train_without_validation_data():
    data = _make_regression_data(with_val=False)
    model = XGBoostRegressor().train(data, learning_rate=0.1, n_estimators=10, random_state=42)
    assert model.model is not None
