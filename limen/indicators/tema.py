...
def tema_from_values(values: npt.NDArray[np.float64], period: int) -> npt.NDArray[np.float64]:
    n = len(values)
    ...
    _, first_ema = ema_talib_default_segment(values, period, lookback_ema, n - 1)
    if first_ema.size == 0:
        return out
    ...
    _, third_ema = ema_talib_default_segment(second_ema, period, 0, len(second_ema) - 1)
    if third_ema.size == 0:
        return out
