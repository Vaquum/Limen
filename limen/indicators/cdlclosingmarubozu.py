...
CDLCLOSINGMARUBOZU_BODY_LONG_AVG_PERIOD = 10
CDLCLOSINGMARUBOZU_BODY_LONG_PERIOD_TOTAL = 0.0
CDLCLOSINGMARUBOZU_SHADOW_VS_AVG_PERIOD = 10
CDLCLOSINGMARUBOZU_SHADOW_VS_FACTOR = 0.1
CDLCLOSINGMARUBOZU_SHADOW_VS_PERIOD_TOTAL = 0.0


def _cdlclosingmarubozu_impl(
    data: pl.DataFrame,
    open_col: str = 'open',
    high_col: str = 'high',
    low_col: str = 'low',
    ...
    body_long_avg_period = CDLCLOSINGMARUBOZU_BODY_LONG_AVG_PERIOD
    shadow_vs_avg_period = CDLCLOSINGMARUBOZU_SHADOW_VS_AVG_PERIOD
    shadow_vs_factor = CDLCLOSINGMARUBOZU_SHADOW_VS_FACTOR
    lookback_total = max(body_long_avg_period, shadow_vs_avg_period)

    out = np.zeros(n, dtype=np.int32)
    if n <= lookback_total:
        return data.with_columns(pl.Series(name='cdlclosingmarubozu', values=out))

    body_long_period_total = CDLCLOSINGMARUBOZU_BODY_LONG_PERIOD_TOTAL
    shadow_vs_period_total = CDLCLOSINGMARUBOZU_SHADOW_VS_PERIOD_TOTAL
    ...
    while i < n:
        ...
        if real_body > body_long_avg and (
            (color == 1 and upper_shadow < shadow_vs_avg)
            or (color == -1 and lower_shadow < shadow_vs_avg)
        ):
            out[i] = color * 100
