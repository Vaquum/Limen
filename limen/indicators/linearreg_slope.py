...
def _linearreg_slope_from_values(values: npt.NDArray[np.float64], period: int) -> npt.NDArray[np.float64]:
    n = len(values)
    ...
    while today <= end_idx:
        ...
        out[today] = ((period * sum_xy) - (sum_x * sum_y)) / divisor
        today += 1
