...
- Trim `governance/_common.py` to the surface Limen's gates import — `__all__ = ['CLOSING_KEYWORD_RE']` and the pattern itself — removing the upstream-template helpers (`CC_RE`, `fail_setup`, `resolve_package_dir`, `significant_lines`, `find_python_files`, `REPO_ROOT`, `TYPING_BUDGET`) that reference gates Limen never adopted; the divergence from the upstream template is documented in the module docstring (#691).
