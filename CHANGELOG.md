...
- Add a zero-error pyright gate: commit a strict `[tool.pyright]` configuration in `pyproject.toml` (correctness rule families held at error, high-volume noise families downgraded to a visible warning backlog), pin `pandas-stubs>=2.3,<2.4`, `scipy-stubs>=1.15,<2`, `pyright>=1.1.408,<1.1.409` (exactly the 1.1.408 audit version, in the bounded-range form the package audit requires), and an unconditional `tomli` in the `dev` extra, and add a `pr_checks_pyright` CI workflow that fails any PR whose merge state reports a single error-level diagnostic; the recorded ratchet baseline at merge is 0 errors and 2,850 warnings in the gate environment (Python 3.10, `.[all,dev]` under `requirements/constraints.txt`; local counts vary with the resolved environment) (#650).
