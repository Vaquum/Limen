...
- Update the slice template's Done Means significance to name rule 10 and the closeout writer, exempt `governance/` from pyright analysis and the sdist manifest check, and pin the gate files, rule markers, workflow triggers, writer markers, and template sentence with `test_slice_gate_and_closeout_surfaces` (#659).
