...
- Close the residual soft edges named in the 2026-07-14 governance review. Replace `.github/CODEOWNERS`' global `* @zero-bang` review-request rule with three enforcement-surface owner lines — `/governance/`, `/.github/`, and `/tests/test_packaging_surface.py`, each owned by both `@mikkokotila` and `@zero-bang` — so that with `require_code_owner_review` enabled on the branch ruleset any future PR touching those paths cannot merge without a non-author code owner's approval, a server-side layer the judged PR's merge ref cannot edit away. Naming both owners on every line keeps either able to approve the other's PR (no single-owner deadlock), and the global rule is dropped because under code-owner review it would have made one owner required on every PR in the repo (#691).
- Add `.github/workflows/pr_checks_slice_sweep.yml`: a scheduled (`cron '17 */6 * * *'`) and `workflow_dispatch` sweep that enumerates every open PR targeting main, fetches each PR's title/body/paginated changed files (hard-failing the posted verdict if the enumerated count disagrees with the PR's canonical `changedFiles`), re-runs `governance/slice_gate.py`, and POSTs a fresh `pr_checks_slice` check-run on the head SHA — the same fail-closed rerun contract as `pr_checks_slice_on_issue.yml`. This bounds the staleness window left open when the event-driven rerun crashes before posting: a stale green now survives at most one six-hour sweep interval (#691).
- Trim `governance/_common.py` to the surface Limen's only gate imports — a docstring naming just the slice gate, `__all__ = ['CLOSING_KEYWORD_RE']`, and the `CLOSING_KEYWORD_RE` pattern. The upstream-template helpers for gates Limen never adopted (`CC_RE`, `fail_setup`, `resolve_package_dir`, `significant_lines`, `find_python_files`, `REPO_ROOT`, `TYPING_BUDGET`) are removed; the divergence from the template is intentional and documented in the module docstring (#691).
- Pin all three with `test_governance_hardening_surfaces` in `tests/test_packaging_surface.py`: the three CODEOWNERS owner lines and the absence of a global rule, the sweep workflow's `schedule:` trigger plus its `name=pr_checks_slice` POST marker and truncation-refusal marker, and `_common.py`'s trimmed surface (`__all__` exactly `['CLOSING_KEYWORD_RE']`, zero `cc_gate` mentions, none of the removed helpers) — so a future PR that regrows any of them reds `PR Checks Tests` (#691).
