...
- Upgrade `slice_closeout_guard` from reopen-on-empty to fill-then-verify: a slice issue closed by a merged PR gets its Merge SHA, PR number, and required-check run ids written into the Done Means evidence fields by the guard and the close stands; reopen remains only for closes with no merged closing PR and no evidence (#659).
