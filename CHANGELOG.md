...
- Add a `skipped` status to the test runner so optional-dependency guards are honest instead of silently green. `execute_test_suite` gains a `skip_exceptions` injection point (stdlib-pure, so the artifact-only `pr_checks_runtime` job keeps importing `runtime_tracking` without pytest installed); `tests/run.py` passes `pytest.skip.Exception`, the profile gains `skipped_count` plus per-test `skip_reason` (additive, old artifacts render as 0), and the runtime summary shows the count. The TabPFN reference test's `if TabPFNBinary is None: return` — which CI reported as *passed* while executing nothing, since `tabpfn` is an optional extra not in `requirements.txt` — now calls `pytest.skip('tabpfn is not installed')`: a real skip under local pytest, a visible `⏭️ SKIPPED` record under `tests.run`
