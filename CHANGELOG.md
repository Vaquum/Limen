...
- Add self-contained-experiment SFD load path to [`Trainer.__init__`](limen/experiment/trainer/trainer.py); the new `_load_sfd_module` helper prefers a `<sfd_module_name>.py` file inside `experiment_dir` (loaded via `importlib.util.spec_from_file_location` + `module_from_spec`, with no `sys.path` mutation and the SFD module itself not registered in `sys.modules`) and falls back to `importlib.import_module` only when no such file exists. Forward-looking flows that ship the SFD inside the experiment_dir (e.g. `trainer_prep.py`-style bundle preparation for paper-trade deploy) now Just Work without operator-side `PYTHONPATH` wiring; built-in SFDs referenced by fully-qualified package path continue to load via the legacy fallback unchanged. The SFD's own top-level imports may still populate `sys.modules` as Python normally does — only the SFD module itself is held outside the global registry
- Add path-traversal hardening to `_load_sfd_module`: rejects any `sfd_module` name whose dot-separated segments are not valid Python identifiers, blocking `..`, `/`, `\`, leading/trailing dots, and empty segments before either branch runs. The legacy dotted package-path fallback (e.g. `limen.sfd.foundational_sfd.logreg_binary`) continues to validate cleanly. The residual `import_module`-side surface (any module installed on `sys.path` is namable) is captured as TD-001 in [`docs/TechnicalDebt.md`](docs/TechnicalDebt.md) for live-trade hardening
- Add 3 test groups pinning the contract — [`test_trainer_loads_sfd_from_experiment_dir_without_sys_path_mutation`](tests/test_trainer.py) stages a tmp dir with a custom SFD `.py` + bare module name and asserts `Trainer(...)` succeeds with `sys.path` unchanged and the bare SFD name not registered in `sys.modules` on exit; [`test_trainer_falls_back_to_import_module_when_no_local_sfd_file`](tests/test_trainer.py) stages a tmp dir referencing `limen.sfd.foundational_sfd.logreg_binary` by package path and asserts the legacy import path still works; [`test_trainer_rejects_path_traversal_and_invalid_module_names`](tests/test_trainer.py) parametrizes 11 hostile or malformed names and asserts each raises `ValueError`
- Add [`docs/TechnicalDebt.md`](docs/TechnicalDebt.md) and TD-001 documenting the residual `import_module` fallback hardening needed before any live-trade deploy
- Update [`docs/Trainer.md`](docs/Trainer.md) to describe the two-stage SFD load (local file then `import_module`), the path-traversal validator, and the trust contract on `experiment_dir`
