...
- Add `split_walk_forward` to `limen.data.utils`: a pure walk-forward splitter returning ordered (train, test) fold pairs over a time-ordered frame, with label-horizon purging (`purge_bars`), post-test embargo (`embargo_bars`), and anchored or fixed-width rolling train windows; degenerate geometries fail loud with `split_walk_forward`-prefixed `ValueError`s instead of returning empty folds. A Hypothesis contract suite (`tests/test_walk_forward_contracts.py`, deterministic CI profile) asserts the purge gap, embargo exclusion, partition, and row-identity laws. First slice of the leakage-safe walk-forward evaluation PRD (#738).
