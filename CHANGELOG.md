...
- **Breaking:** Price the long-flat entry at the signal bar's close. `long_flat_strategy` now computes one close-to-close return per held bar, `close_t / close_{t-1} - 1`, so the entry fill is the close of the signal bar: the price Nexus acts on and the Praxis replay venue fills at. The separate `price_change / open` entry return is removed, and an execution row with no finite, non-zero prior close (the first row under `execution_lag_bars=0`, or the row after a price gap) is non-tradable. Every historical ledger value is re-priced; the shift per entry is bounded by `|open_{t+1} - close_t| / close_t`, a few tenths of a basis point per trade on liquid hourly bars. The strategy signature, `backtest_snapshot` and its validation, the 20-column ledger, the fee and slippage multipliers, `notional_rate`, and `execution_lag_bars` are unchanged (#805).
