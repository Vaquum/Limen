...
- Add `uel.extras` for storing any arbitrary artifacts in `round_results` in `uel.run`
...
- Add `models.logreg` as a base model for logistic regression modeling
...
- Add `get_state` / `set_state` to `ParamDomain` for checkpoint serialization and restore
...
- Add `limen.calibration` submodule with sklearn-based probability calibration and vectorized grid threshold optimization utilities
...
- Add explicit callable protocols for plug-and-play custom calibrators and threshold optimizers
...
- Fix macOS-only failures in `test_yaml_config` and `test_yaml_store`: resolve the temporary directory before comparing it against the canonicalized output of `find_project_root`/`get_store_path`/`resolve_manifest_uri`, so the `/tmp` vs `/private/tmp` symlink no longer causes a spurious path mismatch. Test-only change; no library behavior affected
...
- Make `cusum_filter` lazy-compatible. The feature subscripted the frame (`data[close_col]`), which raises `LazyFrame is not subscriptable` inside the manifest feature pipeline — `_apply_feature_transforms` pipes transforms as Polars LazyFrames, so any YAML manifest listing `limen.features.cusum_filter.cusum_filter` crashed its whole run on round 0. Column access is now an expression (`pl.col(close_col).cast(pl.Float64).map_batches(...)`), mirroring the `fractional_diff` pattern, so the feature runs identically on eager DataFrames and piped LazyFrames; the eager output is byte-identical to before.
- Change the `logreg_binary` reference architecture `n_jobs` default from `-1` to `None` (and the foundational SFD's `'n_jobs': [-1]` preset to `[None]`). Binary classification fits a single class, so sklearn's class-level parallelism routes one task through a joblib/loky worker pool whose sleep-polled dispatch dominates the round: profiling a manifest run showed `LogisticRegression.fit` at 600 ms/call with 4.67 s of 8.54 s total runtime inside `time.sleep`, and an isolated benchmark on the same prepared data (5,393 x 41) measured lbfgs at 1,072 ms median with `n_jobs=-1` against 57 ms with `n_jobs=None` (19x; `liblinear` ignores the knob at ~30 ms either way). Explicit `n_jobs` params in manifests keep working unchanged. Reproducibility note: `liblinear` rounds are byte-identical to before; `lbfgs` coefficients shift within the solver's stopping tolerance because the worker pool changed the floating-point execution path (verified: same-config fits are bit-identical run-to-run, and the old-vs-new coefficient gap shrinks ~8.5x when `tol` drops from 1e-3 to 1e-8) — at `tol=0.001` this moves test metrics by about ±0.003 AUC, and old `-1` results were already pool- and machine-dependent in the same way.
- Cache Hugging Face dataset snapshots on disk at `~/.cache/limen/datasets/{org}--{repo}/{file_name}`. Every fetch previously re-downloaded the full snapshot parquet (1.3-3.2 s for native dollar-bar repos, 60 s for the 452 MB 1M-source aggregation path) and a single mid-body CDN read timeout aborted the whole experiment — the dominant observed failure mode (three incidents in ~20 fetches during profiling) and the root of the CI 429 flake. `latest.json` still resolves fresh on every run and snapshot file names are immutable and date-stamped, so `repo + file_name` is an exact key with no staleness path: a refreshed snapshot publishes a new name, misses the cache, downloads once (atomic unique-temp-file write + rename), and superseded snapshots of the same repo are pruned. Non Hugging Face URLs and local files are never cached. The immutable-file-name convention is documented as a load-bearing invariant on the Vaquum dataset-publishing side (no content check on a hit), and the cache is best-effort: an unreadable or unwritable cache logs a warning and falls back to the direct download instead of failing the fetch.

## [3.29.0] - 2026-06-10

- Port the tradeline long-binary research track (Vaquum/alpha-research, `limen-archive-dec-2025`, `loop/sfm/lightgbm/`) to Limen as YAML-expressible components — the genuine modeling on honest rails, with the archive's full-dataset fitting, in-SFM trade simulation, and gross-of-cost evaluation deliberately left behind (PRD #604, slice #606; deferred remainder in #605)
- Add `find_price_lines` and `filter_lines_by_quantile` to `limen.utils` — vectorized price-line detection (every bar pair with duration in `[1, max_duration_hours)` and absolute close-to-close change of at least `min_height_pct`; positive lines long, negative short) and per-direction height-quantile filtering, shared by the line features and the tradeline target
- Add `price_lines` to `limen.features` — grouped line-context transform that detects lines internally from scalar params (per split under the manifest pipeline, so detection cannot observe other splits) and adds `active_lines`, `hours_since_big_move`, `line_momentum_<m>h`, `trending_score`, and `reversal_potential` in one detection pass, composing the existing per-column building blocks unchanged. Expression-based via `map_batches` into an unnested struct, so it runs on both eager DataFrames and piped LazyFrames
- Add `quantile_price_lines` to `limen.features` — quantile-line counterpart adding `hours_since_quantile_line`, `active_quantile_count`, `quantile_line_density_<d>h`, `quantile_momentum_<m>h`, `avg_quantile_height_<h>h`, and `quantile_direction_bias`. The two span-count columns (`active_lines`, `active_quantile_count`) carry the research design's within-line lookahead and are documented as research-only; the end-event columns are causal
- Add `TradelineLongBinaryTarget` to `limen.targets` — fits the breakout threshold on the training split as the `long_threshold_percentile` percentile of detected long-line heights (raises when the training split yields no long lines), then labels a bar `1` when the forward max of close over `[t, t + lookahead_hours]` reaches the threshold AND the point return at `+confirmation_hours` or `+lookahead_hours` exceeds it. Rows without a full forward window are null (the archive zero-filled truncated windows; null is the honest form)
- Add `lightgbm_binary` to `limen.sfd.reference_architecture` — `LightGBMBinary(ReferenceModel)` with the full `LGBMClassifier` constructor surface as named wrapper params, early stopping against the validation split, the legacy numeric `class_weight` shorthand alongside `'balanced'`/dict passthrough, the standard `prediction_calibration_config` path, and reproducibility pinned by default (`deterministic=True`, `force_row_wise=True`, `random_state=42`; the class is marked `deterministic` for Trainer's exact-match validation). Inputs are coerced to numpy before fit/predict because LightGBM's sklearn wrapper rejects assignment of `feature_names_in_` on DataFrame input
- Add the `lightgbm_binary` foundational SFD and `limen/yaml/templates/lightgbm_binary.yaml` — the tradeline long-binary experiment end-to-end: swept line geometry, train-fitted target, scaler-from-params, feature ablation, isotonic/sigmoid calibration with grid threshold, swept `fee_bps`/`slip_bps`, and the full LightGBM hyperparameter space (the template's params cover the complete architecture signature, enforced by the arch-surface test)

## [3.30.0] - 2026-06-10

- Register the 29 test functions that were defined in `tests/` but never imported into `tests/run.py`, so they had never run in CI (the suite list is hand-maintained and a missed registration fails silently). The recovered set includes all 11 `CausalRollingRobustScaler` unit tests (among them the `no_lookahead` leakage guard), the three Hugging Face snapshot-cache tests, the four logreg wrapper-surface tests, the cusum LazyFrame parity test, and smaller yaml/profiler/indicator-api/csv gaps. Every one was born unregistered — `git log -S` shows none was ever in the list
- Add `tests/test_run_registration.py` as a permanent guard on the registration contract: it AST-parses every `tests/test_*.py` for top-level `test_*` functions and fails when one is neither in the `tests/run.py` suite list, nor invoked by a registered aggregate in its module (the `test_indicators_vs_talib` pattern), nor named in an explicit `PYTEST_ONLY_TESTS` allowlist. The allowlist carries the eight `get_arrow_file` tests (they take pytest's `tmp_path` fixture, which the zero-arg runner cannot supply) and the heavy `test_tabpfn` e2e; a companion test fails when an allowlist entry goes stale or gets registered. The guard caught its own first omission during this PR's validation run
- Replace the live Binance download in `test_conserved_flux_renormalization` — which fetched the full 42 MB `BTCUSDT-trades-2025-05-23.zip` on every CI run, took 6.2 s, and asserted nothing (`_ = conserved_flux_renormalization(...)`) — with `tests/fixtures/binance_trades_btcusdt_2025-05-23_90s.zip`, the first 90 seconds (5,225 real trades, 40 KB) sliced verbatim from that same archive. The test now asserts the conservation contract on three 30 s bars: exact bar timestamps and column layout, total volume and dollar flow preserved through the bucketing, per-bar `vwap·volume = value_sum`, OHLC/vwap range sanity, all six CFR features finite and non-negative, and byte-identical output on a re-run (0.04 s)
- Remove the SSH tunnel steps from `pr_checks_tests.yml` (key agent, `known_hosts`, forwarded `127.0.0.1:8123`): nothing in `limen/` or `tests/` references that port or any localhost endpoint, so the steps only spent secrets and added a failure surface on every PR run
- Add behavioral tests for the two untested feature modules — `lagged_features` (lag-range/columns/alias surface and its validation errors) and `breakout_features` (exact lag, rolling-stat, and ROC values on a hand-computed frame, including the zero-denominator ROC branch and warmup-row dropping; 18% covered before) — and targeted tests for the cohort selector branches the e2e path never reaches: `top_n`/`backtest_pareto`/`diverse_metrics` input validation, id coercion and rejection, empty-after-filter returns, Pareto dominance with deterministic tie-breaking, degenerate normalization (infinite and constant metrics), and the `diverse_metrics` single-cluster medoid plus score top-up path asserted against hand-computed selections
...
- Add a zero-error pyright gate: commit a strict `[tool.pyright]` configuration in `pyproject.toml` (correctness rule families held at error, high-volume noise families downgraded to a visible warning backlog), pin `pandas-stubs>=2.3,<2.4`, `scipy-stubs>=1.15,<1.16` (the minor line the gate environment resolves against scipy 1.15), `pyright>=1.1.408,<1.1.409` (exactly the 1.1.408 audit version, in the bounded-range form the package audit requires), and an unconditional `tomli` in the `dev` extra, and add a `pr_checks_pyright` CI workflow that fails any PR whose merge state reports a single error-level diagnostic; the recorded ratchet baseline at merge is 0 errors and 2,850 warnings in the gate environment (Python 3.10, `.[all,dev]` under `requirements/constraints.txt`; local counts vary with the resolved environment) (#650).
- Fix all 154 baseline error-level diagnostics across the `limen/` tree with behavior-preserving edits — numpy→builtin coercions at declared boundaries, casts and `isinstance` narrowing at untyped pandas/sklearn/statsmodels/polars boundaries, honest `| None` declarations with source-prefixed narrowing raises for the manifest None-sentinel init pattern, else-raise closure of unbound-variable chains, and annotation corrections where declarations were wrong — including the named latent defects in `focus_reducer.py` (`float | None` ordering comparison), `_experiment_backtest_results.py` (`numpy.bool_` returned as `bool`), `indicators/ma.py` (possibly unbound `expr`), `cli/commands/fork.py` (`str | None` into `re.match`), `utils/param_space.py` (`None` return declared `dict`), and `_permutation_confusion_metrics.py` (Series annotations on ndarray values) (#650).

## [4.3.1] - 2026-07-08

- Add the `slice_closeout_guard` CI workflow: closing any `slice`-labeled issue with unchecked Done Means boxes or an empty `Merge SHA:` reopens the issue with a comment naming the missing fields, so no slice can sit closed without closeout evidence (#653).
- Add a pyright warning ratchet to `pr_checks_pyright`: the single pyright run emits `--outputjson` and an evaluation step fails the required check on `errorCount > 0` or `warningCount > PYRIGHT_WARNING_BASELINE` (committed value `2850`, the gate-environment count recorded at PR #651; lowered in the same PR whenever warnings are reduced), making the warning backlog monotonically non-increasing on `main` (#653).
- Name the guard in the slice template's Done Means significance and pin the guard workflow, the ratchet baseline, and the template sentence with `test_slice_closeout_guard_and_ratchet_surfaces`, so the enforcement itself cannot be silently removed (#653).

## [4.4.0] - 2026-07-08

- Retire the ten mechanical pyright warning families (339 warnings across 98 files) with runtime-inert edits: merge adjacent string literals, rename cross-module `_`-prefixed helpers to public names inside their private modules, add `super().__init__()` to flagged `__init__`s, bind discarded fluent-builder and side-effect call results to `_`, make the three lazy-export `__init__.py` `__all__` lists statically visible via `TYPE_CHECKING` import blocks, widen boundary parameter annotations to the honest incoming type while keeping every runtime guard, declare `UniversalExperimentLoop`'s late-bound instance variables in `__init__` with honest types, hoist call-expression parameter defaults to module constants, and break the `calibration/pipeline.py` ↔ `experiment/manifest_core.py` import cycle with a structural `CalibrationConfigProtocol` (#656).
- Promote the ten families to `error` in `[tool.pyright]` — four strict defaults (`reportPrivateUsage`, `reportUnsupportedDunderAll`, `reportUnusedFunction`, `reportUnnecessaryIsInstance`) plus zero-occurrence `reportUnnecessaryComparison` by deleting their downgrade lines, six off-by-default extras by setting them to `"error"` — leaving exactly eight warning downgrades (the Unknown/stubs/override cluster) (#656).
- Lower `PYRIGHT_WARNING_BASELINE` from `2850` to `2495` in `pr_checks_pyright` — the ratchet's first downward click; the honest instance-variable declarations retired 16 Unknown-family warnings beyond the 339, and the baseline pins the measured gate-environment count at zero headroom (#656).
- Extend `test_pyright_gate_config` to pin the promotions: the six extras assert `== "error"`, the five strict defaults assert absent-or-`error`, and the remaining warning-downgrade set asserts equal to the eight named families, so no future PR can silently demote a promoted rule (#656).

## [4.5.0] - 2026-07-08

- Adopt the organization's governance slice gate from `Vaquum/new-repository-template@c4a7a05` — `governance/slice_gate.py`, `governance/_common.py`, `governance/__init__.py`, run by the new `pr_checks_slice` workflow on every PR event and re-posted by `pr_checks_slice_on_issue` when a linked issue mutates (the rerun set includes sibling slice PRs, whose rule-9 verdicts depend on the mutated issue) — enforcing the eight mechanical PR↔slice rules: exactly one open slice issue cited via Closes/Fixes/Resolves, resolvable and not a PR, title byte-equality, every template Significance blockquote verbatim in the issue body, and the PR diff inside the issue's Surfaces globs and outside its Out of Scope globs — hardened per dual review against issue-form `###` headings (section terminators) and CRLF issue bodies (rule 6 normalisation) (#659).
- Extend the gate with rule 9 (PRD closure: the PR's closing set must be exactly {slice} while the slice's parent PRD has other open slice sub-issues, and exactly {slice, parent PRD} when the slice is the parent's last open one, resolved via the native sub-issues API) and rule 10 (every Done Means checkbox must be `[x]` or carry `OVERRULED: <reason>`; the post-merge evidence fields are structurally exempt), both fed back upstream (#659).
- Upgrade `slice_closeout_guard` from reopen-on-empty to fill-then-verify: a slice issue closed by a merged PR gets its Merge SHA, PR number, and required-check run ids written into the Done Means evidence fields by the guard and the close stands; reopen remains for closes with no merged closing PR and no evidence, and for writer failures, so no close rests without evidence (#659).
- Update the slice template's Done Means significance to name rule 10 and the closeout writer, drop the template's evidence checkbox now that the writer owns the evidence fields, exempt `governance/` from pyright analysis and the sdist manifest check, and pin the gate files, rule markers, workflow triggers, writer markers, and template sentence with `test_slice_gate_and_closeout_surfaces` (#659).

## [4.6.0] - 2026-07-09

- Annotate the indicator and feature layer's own signatures — every parameter and return flagged by `reportUnknownParameterType` or `reportMissingTypeArgument` across `limen/indicators` (39 files), `limen/features` (12), `limen/metrics` (5), `limen/targets/triple_barrier.py`, and `limen/scalers/rank_gauss_scaler.py` — with the honest narrowest types: `npt.NDArray[np.float64]` for the float64 indicator array pipeline, `npt.NDArray[np.int8]`/`np.bool_`/`np.intp` where the array construction says so, `list[dict[str, float]]` for the price-line dicts, `list[int]`/`list[float]`-or-array unions for the metric inputs, and `dict[str, Any]` only for the genuinely heterogeneous data and results containers, driving both rules to zero occurrences within the slice scope (314 direct warnings) (#662).
- Lower `PYRIGHT_WARNING_BASELINE` from `2495` to `1839`: the typed signatures cascade through downstream inference and retire 656 warnings, 342 beyond the direct count; the one revealed error-level diagnostic (`np.quantile`'s `floating[Any]` return against the scaler's `float64` cache declaration) is fixed by widening `RankGaussScaler.quantiles` to the honest `npt.NDArray[np.floating[Any]]` (#662).

## [4.7.0] - 2026-07-10

- Annotate the sfd, utils, calibration, and data layers' own signatures — every parameter and return flagged by `reportUnknownParameterType` or `reportMissingTypeArgument` across `limen/sfd` (12 files, excluding the two pandas reference-architecture files owned by slice 3), `limen/utils` (5), `limen/calibration` (3), and `limen/data/utils/splits.py` — with the honest narrowest types: `dict[str, Any]` for the heterogeneous data and results dictionaries the reference architectures pass end to end, `npt.NDArray[Any] | pl.DataFrame` for calibration validation features and `npt.NDArray[Any] | pl.Series` for labels (the LogReg path ships polars straight through where LightGBM/TabPFN convert) with `npt.NDArray[np.floating[Any]]` for probability vectors, `Callable[[Any, Any], float]` for the threshold metric, `list[dict[str, float]]` for the price-line dicts with `npt.NDArray[np.floating[Any]] | npt.NDArray[np.integer[Any]]` for the coerced close-price input, `list[pl.DataFrame]`/`list[datetime] | list[int]` for the split pipeline, and `dict[str, list[Any]]` for the foundational param spaces, driving both rules to zero occurrences within the slice scope (204 direct warnings) (#663).
- Lower `PYRIGHT_WARNING_BASELINE` from `1839` to `1278`: the typed signatures cascade through downstream inference and retire 561 warnings, 357 beyond the direct count; the annotations reveal no error-level diagnostics, with `split_data_to_prep_output`'s output dictionary declared `dict[str, Any]` at the literal so the typed `_alignment` sub-dictionary assigns cleanly, and `apply_confidence_filtering` widened to accept the `np.floating[Any]` threshold its calibration sibling produces (#663).

## [4.7.1] - 2026-07-10

- Add `tests/conftest.py` capping OpenMP to one thread on macOS (`os.environ.setdefault('OMP_NUM_THREADS', '1')` under `sys.platform == 'darwin'`): the full suite previously deadlocked at `test_tabpfn_train_evaluate_end_to_end` when the xgboost/lightgbm tests loaded one libomp runtime and TabPFN's torch loaded a second; conftest sets the cap before any test module import initializes either runtime, covers both `python -m tests.run` and direct pytest invocations, preserves explicit overrides, and leaves the Linux CI runners untouched (#667).
- Pin the cap with `tests/test_conftest_surface.py::test_darwin_openmp_thread_cap_is_pinned` so removal fails `pr_checks_tests` mechanically — CI runs on Linux and would never exercise the deadlock itself (#667).

## [4.8.0] - 2026-07-11

- Annotate the remaining signatures flagged by `reportUnknownParameterType` or `reportMissingTypeArgument` — `limen/experiment` (5 files), `limen/yaml` (2), `limen/cli/git_utils.py`, and the fifteen pandas-facing files (signature level only; the body rewrites stay with #643) — with the honest narrowest types: `dict[str, Any]` end to end for the reference-architecture data/results dictionaries (`ReferenceModel.train/predict/evaluate` now match the six callers annotated in #663), `npt.NDArray[np.integer[Any] | np.floating[Any]]` for binary prediction arrays, `npt.NDArray[np.bool_]` for masks, `pd.Series | npt.NDArray[Any]` unions for the confusion metric inputs, `dict[str, float]` for confusion and backtest-split returns matched invariantly to `rule_based_metrics`, `Callable[..., Any]` for the frame-polymorphic feature/indicator transforms (they run on `pl.DataFrame` eagerly and `pl.LazyFrame` in the pipe pipeline), `Callable[..., dict[str, Any]]` for architecture functions and the prep/model hooks with `Callable[[], dict[str, Any]]` for params, `Callable[[pl.DataFrame, Any], pl.DataFrame]` for the inverse scaler, a six-element `date | Any` tuple for `split_dates` so the `split_by_dates(*)` splat stays arity-checked, `list[datetime] | list[int]` for the datetime pipeline, `dict[Any, int] | dict[str, dict[Any, int]]` for `MSQ.distribution`, `set[str]` for the YAML rule vocabularies, and `subprocess.CompletedProcess[str]` for `run_git`, driving both rules to zero occurrences repo-wide (189 direct warnings) (#664).
- Lower `PYRIGHT_WARNING_BASELINE` from `1278` to `641`: the typed signatures cascade through downstream inference and retire 637 warnings, 448 beyond the direct count; the one revealed error-level diagnostic (pandas `Series.to_dict` returning `dict[Hashable, Any]` against `RuleBasedStrategy._backtest_split`'s `dict[str, float]` declaration) is fixed with a cast at the pandas boundary (#664).
- Promote `reportUnknownParameterType` and `reportMissingTypeArgument` to their strict-default `error` severity by deleting both downgrade lines from `[tool.pyright]`, and extend `test_pyright_gate_config` to pin the promotion: both rules join the absent-or-`error` assertions and the warning-downgrade set asserts equal to the six remaining families (`reportImplicitOverride`, `reportMissingTypeStubs`, and the Unknown Variable/Member/Argument/Lambda cluster) — phase 3 of the warning burn-down complete, closing PRD #661 (#664).

## [4.9.0] - 2026-07-11

- Add `typing_extensions>=4.12,<5` as a runtime dependency (PSF-2.0, pure Python, no transitive dependencies, recorded in `THIRD_PARTY.md`) and decorate every method that overrides a base-class method with `@override` imported as `from typing_extensions import override` — the Python 3.10-compatible source; when the Python floor reaches 3.12 the import moves to `typing` and the dependency drops — across the 50 flagged sites: the reference architectures' `train`/`predict`/`evaluate` (7 files), the reducers' `analyze_and_intervene`/`get_state`/`set_state` (5 files), the search strategies (2 files), the ML and rule-based manifest hooks, `ForwardVolNormalizedReturnTarget.transform`, and the yaml profiler's logging `Handler.emit`, so a base rename or signature drift that silently stops a method overriding becomes a required-check failure instead of a latent bug (#671).
- Promote `reportImplicitOverride` from `"warning"` to `"error"` in `[tool.pyright]` and extend `test_pyright_gate_config` to pin the promotion: the rule joins the `== "error"` extras, the warning-downgrade set asserts equal to the five remaining families (`reportMissingTypeStubs` and the Unknown Variable/Member/Argument/Lambda cluster), and the runtime dependency pin gains its own assertion (#671).
- Lower `PYRIGHT_WARNING_BASELINE` from `641` to `591` in `pr_checks_pyright`: the 50 decorated sites retire the rule's whole count with no cascade, and the baseline pins the re-measured gate-environment count at zero headroom — phase 2 of the warning burn-down complete, closing PRD #670 (#671).
...
- Type the third-party boundaries flagged by the four `reportUnknown*` families — `reportUnknownMemberType`, `reportUnknownVariableType`, `reportUnknownArgumentType`, `reportUnknownLambdaType` — across `limen/yaml` (197 warnings), `limen/experiment` (91), `limen/cli` (31), and `limen/data` (27) with the house boundary techniques: typed intermediates declaring what the surrounding validation guarantees (`dict[str, Any]` YAML/JSON mappings, `list[Any]` parameter lists, `dict[Any, Any]` where `SfdParams` runtime-validates key types itself), casts at isinstance-guarded seams (`split_config` tuples, checkpoint metadata, intervention filter params), local defs with annotated parameters replacing the two unannotatable manifest prep/model lambdas, a `dict[str, Callable[[dict[str, Any]], Any]]` declaration typing the eleven intervention-dispatch lambdas through bidirectional inference, `field(default_factory=list[str])`-style parameterized factories for the dataclass fields, and declared `Callable[..., dict[str, Any]] | None` prep/model attributes on `UniversalExperimentLoop`, driving all four rules to zero occurrences within the slice scope (346 direct warnings) (#673).
- Add shared typed facades where an untyped library kills inference at every member access: `RoundTripYAML` Protocol with a `round_trip_yaml()` factory plus `is_mapping`/`is_list` TypeGuards in `limen/yaml/config.py` restating ruamel.yaml's own declared `load -> Any`/`dump` contract (ruamel guards its typing imports under `if False:  # MYPY`, so pyright reads every ruamel member as Unknown despite `py.typed`), consumed across the yaml store, parser, rules, compiler, the cli commands, and the experiment JSON seams; `_PCATransformer` Protocol with a `_pca_transformer()` factory in `manifest_core.py` for the sklearn PCA compression seam; and `_Arrow*` Protocols with `_arrow()`/`_arrow_ipc()` module facades in `historical_data.py` for the compiled pyarrow zero-copy validation surface — each facade returned through a function so the declared Protocol type survives (direct annotation narrows back to the concrete class on assignment) (#673).
- Lower `PYRIGHT_WARNING_BASELINE` from `591` to `245` in `pr_checks_pyright`: the four directories retire exactly their 346 direct warnings with zero cascade and zero revealed error-level diagnostics, and the baseline pins the re-measured gate-environment count at zero headroom (#673).

## [4.11.0] - 2026-07-13

- Type the remaining `reportUnknown*` residue — 228 warnings across `limen/scalers` (47), `limen/sfd` (45), `limen/utils` (29), `limen/features` (24), `limen/metrics` (19), `limen/cohort` (44), `limen/inference` (8), `limen/log` (8), `limen/backtest` (2), `limen/calibration` (1), and `limen/indicators` (1), including cast-level-only work in the pandas-facing files (#643 keeps the body rewrites) — with the slice-1 techniques: declared accumulator intermediates (`list[pl.Expr]` scaler expressions, `list[pd.DataFrame]` log rows, `list[npt.NDArray[np.float64]]` score parts, `list[tuple[float, int]]` event sweeps), `_is_integral`/`_is_floating` TypeGuards fusing the `(int, np.integer)`/`(float, np.floating)` isinstance guards with the honest parameterized narrow in the three cohort selectors, identity casts at isinstance-guarded JSON and pandas seams, typed `dict[str, Callable[[pl.Expr, Any], pl.Expr]]` rule-based predicate lambdas, declared DLinear weight attributes, and a module-level def replacing the one unannotatable manifest feature lambda, driving all four rules to zero occurrences repo-wide (#674).
- Add typed facades over the remaining untyped third-party surfaces, each returned through a factory function so the declared Protocol survives assignment narrowing: sklearn `StandardScaler`/`PCA`/`KMeans` in the diverse-metrics selector, `LogisticRegression` in the logreg reference architecture, `CalibratedClassifierCV` in probability calibration, per-file `sklearn.metrics` module facades (binary, continuous, multiclass, balanced, safe-OVR-AUC, confidence filtering), and a lazy `statsmodels.tsa.stattools` module facade for the ADF test — resolvable modules are structurally checked against their inferred signatures, so `zero_division` stays `Any` (sklearn's own default is the string `'warn'`), `roc_auc_score`/`adfuller` return `Any` behind the pre-existing honest call-site casts, and `autolag` is keyword-only; the two revealed error-level diagnostics (polars `Series.mean` returning `PythonLiteral | None` against the logreg scaler's `dict[str, float]`, and the confidence threshold's `np.floating[Any]` declaration) are fixed with boundary casts and the honest numpy scalar annotation (#674).
- Close the pyright warning ratchet at zero: the four `reportUnknown*` downgrade lines are deleted from `[tool.pyright]` (strict-default `error`), `reportMissingTypeStubs` is set to `"none"` as the campaign's single documented exception (the rule reports the absence of third-party stub packages for sklearn, statsmodels, lightgbm, xgboost, and ta-lib — fixable only by vendoring stubs, rejected as a permanent maintenance surface), `PYRIGHT_WARNING_BASELINE` reaches `0`, and `test_pyright_gate_config` pins the terminal shape: the Unknown quartet joins the absent-or-`error` assertions, the warning-downgrade set asserts empty, the `"none"` set asserts exactly `{'reportMissingTypeStubs'}`, and the baseline line asserts literal `0` — the gate's steady state becomes `errors=0 warnings=0 baseline=0`, closing the campaign at 2,850 → 0 across four phases (#674).

## [5.0.0] - 2026-07-13

- **Breaking:** rewrite `limen.backtest.backtest_snapshot()` from a pandas-DataFrame-in/DataFrame-out ledger to a `Mapping[str, Any]`-in/`dict[str, float]`-out one. Callers that duck-typed a `pd.DataFrame` as the `columns` input still work; callers reading the previous one-row `pd.DataFrame` result (e.g. `.iloc[0]['col']`) must switch to plain dict access (`result['col']`). `long_flat_strategy()` and `ExecutionResult(pos, gross, net)` follow suit — inputs are duck-typed via `np.asarray` (pandas `Series`, lists, and arrays all still work), but the returned `pos`/`gross`/`net` fields are now `npt.NDArray[np.float64]` instead of `pd.Series`, so index-based access on the result no longer applies. The pandas-index full-window validation contract is replaced by an explicit length check, and `columns` missing a required key or carrying mismatched column lengths now raises a specific `ValueError` instead of a `KeyError` or a misleading "requires at least one row". `avg_win_bps`/`avg_loss_bps`/`cvar_95_pnl_bps` keep their NaN-on-empty sentinel behavior; percentiles keep `np.quantile`'s linear interpolation (never polars' `nearest` default) so `edge_bps_p5/p50/p95`, `pnl_bps_*`, `cost_bps_*`, and `drawdown_bps_*` are bit-for-bit unchanged. Verified behavior-preserving with a golden-master harness (14 edge cases — flat/all-in/alternating positions, no-winners/no-losers, single/two-bar windows, execution-lag and notional-rate sweeps, fee/slip combinations, NaN prices, a ≥20-bar CVaR-active window — captured from the prior pandas implementation) plus 3,000-trial differential fuzzing against it, both reporting zero mismatches (#378).
- **Breaking:** convert the `limen.cohort.sfc` selectors (`top_n`, `backtest_pareto`, `diverse_metrics`) and `Cohort._build_selector_context`'s `results.csv` load from pandas to polars. `context['results']` passed to a custom selector is now a `polars.DataFrame` (`pl.read_csv(..., infer_schema_length=None)`), and the three built-in selectors reject a pandas `DataFrame` with `ValueError` where they previously required one. `coerce_id`'s missing-value check moves from `pd.isna` to an explicit `None`/`NaN` check with identical semantics. `diverse_metrics`' IQR outlier trim keeps `np.quantile`'s linear interpolation (not polars' `nearest` default) so the PCA/KMeans medoid selection is unaffected. Verified with 2,900-trial differential fuzzing (NaN/inf metrics, tied scores, varying `min_signals`/`iqr_multiplier`/`n_clusters`/`target_count`) against the prior pandas implementation — zero mismatches, including identical error behavior. pandas now remains only in `limen/log/` (`Log`, `experiment_parameter_correlation`, `experiment_backtest_results`, `permutation_confusion_metrics`), which depends on pandas-only features (MultiIndex results, `.corr(method='kendall')`, ordered-Categorical cohorts, a label-aligned bootstrap) with no polars equivalent (#378).
- Add a declarative `uel.pruning_strategies` YAML block: each entry names a reducer by its `REDUCER_REGISTRY` key (`budget`, `correlation`, `focus`, `sanity`, `saturation`) and forwards `params` to its constructor. `PruningStrategiesSpec` validates the block (list shape, known reducer types, unknown fields, `params` must be a mapping) before `limen validate`/`limen run` proceeds; `build_pruning_strategies()` compiles it into concrete reducer instances, mirroring the existing `build_search_strategy()` pattern, and is wired into both `limen run` and `limen run --resume` so a YAML experiment gets the same feedback control previously only reachable through the Python API. `limen run` now constructs the search strategy, pruning strategies, and compiled SFD before creating the results directory, so a malformed `pruning_strategies` block reports a clean CLI error instead of a raw traceback with an orphaned results directory left behind.
- Fix `uel.search_strategy.seed`: add it to the YAML schema's `search_strategy` allowlist so a seeded `random` search no longer produces a spurious "unknown field" validation warning; the compiler already read and applied it.
- Add three new guides — [Perturbation Strategies](docs/Perturbation-Strategies.md) (the five parameter-controlled perturbation primitives, when to use each by model type, and deriving feature importance from an ablation sweep), [Fractional Differentiation](docs/Fractional-Differentiation.md) (stationarity, `find_min_d`/ADF, and wiring `fractional_diff` into a manifest or YAML experiment), and [Triple-Barrier Method](docs/Triple-Barrier-Method.md) (the three barriers, sizing the reward-to-risk ratio, and attaching `TripleBarrierTarget`) — and route them from the README, docs hub, and `docs-site/scripts/assemble-docs.mjs`.
- Fill documentation gaps surfaced by an audit of issue #378 (WP-Limen-0009): inline-metrics coverage in [Universal Experiment Loop](docs/Universal-Experiment-Loop.md), a reducer selection matrix and tuning guidelines plus the new YAML `pruning_strategies` and `intra_callback` examples in [Reducers And Feedback](docs/Reducers-And-Feedback.md), a perturbation-impact analysis section in [Log](docs/Log.md), and a `uel` execution-options reference table in [Experiment Manifest](docs/Experiment-Manifest.md). Correct two stale claims: [Cohort](docs/Cohort.md) now states `context['results']` is a polars DataFrame, and `limen/backtest/README.md` now correctly describes snapshot outputs as basis-point scaled (not percent units).
- Add integration test coverage that was previously only exercised in isolation: a real reducer wired through `FeedbackController` and driven by the live feedback cycle actually trims a `UniversalExperimentLoop` run; `GridStrategy` under live feedback re-plans its domain and stops yielding a removed value; and a single chained test covering `with_params_override()` → `prepare_data()` → class-based target training → inline-metrics emission. Add a `test_yaml_code_fences_parse` guard (sibling of the existing Python-fence test) that `yaml.safe_load`s every ` ```yaml ` fence across `README.md`, `docs/`, and package `README.md` files, so doc-embedded YAML examples — including the new reducer configuration example — can't silently rot (#378).

## [5.1.0] - 2026-07-14

- Heal the PyPI publish pipeline: bump past the burned `5.0.0` filename (PyPI permanently rejects a previously used filename, so GitHub stood at v5.0.0 while PyPI served 4.11.0 and every publish retry re-failed) and add a pre-build filename-availability guard to `pr_publish_pypi.yml` that queries the PyPI JSON API and fails loud, with the burned filenames named, before anything is built or uploaded (#687).
- Confine the release model to prose: `scripts/create_release.py` computes the tag from `pyproject.toml`, validates it against `TAG_RE` (`^v\d+\.\d+\.\d+$`), rejects any model deviation on `tag` or `version`, requires the version's `CHANGELOG.md` entry before tagging, and appends mechanical traceability (merged pull requests since the previous tag, compare link, changelog anchor); the new offline `tests/test_release_contract.py::test_release_tag_authority_and_traceability` pins the boundary (#687).
- Gate the tag-to-publish path: the publish job runs behind the `pypi` GitHub environment (mirrored in the PyPI trusted-publisher configuration), the resolved tag is re-validated against the release-tag shape and cross-checked against the built version, tag names reach shell steps only through `env:` indirection, and a `v*` tag ruleset restricts release-tag creation, update, and deletion to the GitHub Actions integration (settings evidence in the slice closeout) (#687).
- Ship release deliverables: after a successful PyPI publish the workflow attaches the wheel, sdist, and a CycloneDX `sbom.json` to the GitHub release and appends their SHA-256 digests to the release body, replacing the dead `attach_release_assets` job that never ran on the live `workflow_run` path (#687).
- Harden every workflow: least-privilege top-level `permissions:` with job-scoped elevations, full commit-SHA pins for every action reference, `persist-credentials: false` on every checkout except the release tag push, and CI tool installs constrained by `requirements/constraints.txt`, whose mirror invariant in `test_constraints_mirror_runtime_envelope` now spans the `test` extra and the bare CI tools (`build`, `cyclonedx-bom`, `pip-audit`) with extras stripped, because constraints files reject extras (#687).
- Add the dependency vulnerability gate: `governance/check_dependency_vulnerabilities.py` (adopted from Vaquum/new-repository-template@c4a7a05, adapted for Python 3.10) runs pip-audit over `[project.dependencies]` in the new `pr_checks_supply` required check; a CVE blocks merge unless `.github/vuln_exceptions.json` carries an `id` + `reason` + `expiry` entry, an expired exception fails again, and `.github/dependabot.yml` (pip and github-actions ecosystems, weekly) keeps the pins fresh (#687).
- Extend `tests/test_packaging_surface.py` with `test_supply_chain_surfaces` — every workflow declares `permissions:`, zero mutable-tag `uses:` references, the checkout credential policy, the dependabot and vuln-gate files, the publish guard/environment/asset-upload surfaces, and `TAG_RE` in the release script — and add `docs/Developer/Release-Policy.md`, the repo-local publish-path contract, routed into the docs site (#687).
...
- Replace the global `* @zero-bang` rule in `.github/CODEOWNERS` with ownership of the enforcement surfaces — `/governance/`, `/.github/`, and `/tests/test_packaging_surface.py`, each owned by `@mikkokotila` and `@zero-bang` — and enable `require_code_owner_review` on branch ruleset 5406995 via the API, so a PR touching the files that decide merge verdicts cannot merge without an approval from a non-author code owner: a server-side layer the judged PR's own merge ref cannot edit. The global rule had to go because under `require_code_owner_review` it would have made `@zero-bang` a required approver for every PR, not only the enforcement surfaces; explicit reviewer requests replace the auto-request convenience it provided (#691).
- Add `.github/workflows/pr_checks_slice_sweep.yml`, a scheduled rerun (every six hours, plus `workflow_dispatch`) of `governance/slice_gate.py` for every open PR targeting main, executed from a checkout of main and reposting each PR's `pr_checks_slice` check-run on its head SHA with the same fail-closed truncation contract as the on-issue rerun, so a stale gate verdict survives at most one sweep interval even if `pr_checks_slice_on_issue` crashes before posting (#691).
- Trim `governance/_common.py` to the surface Limen's gates import — `__all__ = ['CLOSING_KEYWORD_RE', 'REPO_ROOT', 'fail_setup']`: the closing-keyword pattern for the slice gate plus the repo root and setup-failure reporter for the dependency vulnerability gate that arrived with v5.1.0 — removing the upstream-template helpers (`CC_RE`, `resolve_package_dir`, `significant_lines`, `find_python_files`, `TYPING_BUDGET`) that reference gates Limen never adopted; the divergence from the upstream template is documented in the module docstring (#691).
- Add `test_governance_hardening_surfaces` to `tests/test_packaging_surface.py`, pinning the three CODEOWNERS owner lines, the sweep workflow's cadence trigger, `pr_checks_slice` check-run POST and truncation-refusal marker, and `_common.py`'s trimmed `__all__` surface inside the required `PR Checks Tests` gate (#691).
- Delete branch ruleset 12529363, a weaker near-duplicate of Protect-Main (5406995), after verifying every rule it carries exists at equal or stricter strength in 5406995 — deletion and non_fast_forward identical, pull_request at 0-vs-1 required approvals with a merge-method superset, copilot_code_review identical — leaving exactly one active ruleset as the branch law (#691).

## [5.3.0] - 2026-07-15

- Adopt the Conventional Commits gate from the org template: `governance/cc_gate.py` (source `Vaquum/new-repository-template@405b84d3`, provenance in the docstring) enforces CC v1.0.0 on the PR title, on every non-merge commit subject in the PR's commit range, and — diverging deliberately from upstream's single-reference short-circuit — on the title of every closing-referenced issue carrying the `slice` label, so rule-9 last-slice PRs (whose closing set is {slice, parent PRD}) no longer skip slice-title validation while PRD titles stay exempt; the divergence is filed upstream as new-repository-template#61 (#709).
- Run the gate on every PR via `.github/workflows/pr_checks_cc.yml`, adapted to Limen law: Python 3.10 CI parity, SHA-pinned actions with `persist-credentials: false`, least-privilege permissions, and `fetch-depth: 0` for the commit-range scan; combined with slice-gate rule 5 (PR title byte-equals the slice title), slice titles are confirmed Conventional Commits and PR titles inherit it. The gate also carries upstream's AI/LLM-attribution scan: commit subjects and messages in the range must not name an AI assistant or carry AI attribution trailers (#709).
- Restore `CC_RE` to `governance/_common.py` — now with a real consumer — making its surface `__all__ = ['CC_RE', 'CLOSING_KEYWORD_RE', 'REPO_ROOT', 'fail_setup']`, and rewrite `.github/ISSUE_TEMPLATE/slice.yml`'s title guidance from the noun-phrase form to the CC subject form so authors write gate-passing slice titles (#709).
- Pin the new surfaces with `test_cc_gate_surfaces` in `tests/test_packaging_surface.py` — gate file and its `_common` import, workflow triggers and invocation markers, the four-name `_common.py` surface, the template guidance marker, and a live subprocess run of the gate on a passing case — and update `test_governance_hardening_surfaces` to the restored surface (#709).

## [5.4.0] - 2026-07-15

- Attach the GitHub build-provenance Sigstore bundle to every release as the `provenance.intoto.jsonl` asset: `pr_publish_pypi.yml` stages the `actions/attest-build-provenance` bundle output in `build_distribution`, ships it through a `release-provenance` artifact, uploads it beside the wheel, sdist, and `sbom.json`, and covers it in the release body's SHA-256 digest block, so a consumer can verify a downloaded artifact online (`gh attestation verify <artifact> --repo Vaquum/Limen`) or fully offline against the bundle (#712).
- Add `docs/Roadmap.md`, the issue-anchored twelve-month roadmap (2026-07 through 2027-06) ranking seven delivery themes by contribution to credible alpha discovery, naming the leakage-safe evaluation and statistical acceptance gates that `docs/Benchmark.md` disclaims today as committed intent, and stating explicit non-goals and its own review cadence; assembled at `overview/roadmap` and linked from the product home and docs hub (#712).
- Add `docs/Developer/Security-Assurance-Case.md`, the requirement-by-requirement argument for Limen's security posture — threat model, trust boundaries, six requirements each with argument and mechanical evidence, residual risks, and verification commands; assembled at `developer/security-assurance-case` (#712).
- Commit in `SECURITY.md` to crediting vulnerability reporters in the fix's release notes and changelog entry unless they request otherwise, and document release-artifact verification with a pointer to the release policy's new `Verifying release artifacts` section, which `docs/Developer/Making-Release.md` mirrors in its deliverables and evidence lists (#712).
- Pin the new surfaces inside required gates: `test_supply_chain_surfaces` reds a PR that drops the provenance staging, upload, or digest coverage; `test_site_assembly_preserves_authoring_and_navigation_contracts` and `test_documentation_corpus_is_fully_routed_into_the_site` red one that removes either new page or its route; `test_public_risk_and_install_contracts_remain_visible` reds one that removes the reporter-credit or verification commitments (#712).

## [5.4.1] - 2026-07-15

- Default the release-notes model in `scripts/create_release.py` to `claude-sonnet-5` (from `claude-opus-4-6`): the release model composes prose only — every identifier is computed mechanically and pinned by `tests/test_release_contract.py` — so the Sonnet tier is sufficient and roughly 40% cheaper per token; `ANTHROPIC_MODEL` remains the override (#715).
- Disable thinking explicitly in the release script's API call: `claude-sonnet-5` runs adaptive thinking when the parameter is omitted, which would lead the response with a thinking block (breaking the script's first-content-block text extraction) and spend thinking tokens against the 4096-token output budget. The parameter is gated on the model: overrides matching `OMIT_THINKING_MODEL_PREFIXES` (`claude-fable`, `claude-mythos`) omit it entirely via the SDK's `NOT_GIVEN` sentinel, because those models reject an explicit disabled configuration (#715).
- Pin the new default inside the required `PR Checks Tests` gate: `test_supply_chain_surfaces` reds a PR that reverts the script to an Opus model, drops the explicit thinking configuration, or reintroduces an Opus reference (#715).

## [5.5.0] - 2026-07-15

- Add `tests/test_property_contracts.py`, Hypothesis property-based tests over three contractual surfaces: the manifest validator's `uel.n_permutations` fail-loud rules (generated bool/none/string/float/non-positive families all rejected; in-budget integers accepted and over-budget rejected against the template's parameter space), `ParamSpace` cardinality laws (full enumeration equals the cartesian product without duplicates; sampling is unique and seed-deterministic), and `split_sequential` partition invariants (no row lost, duplicated, or reordered for generated frame lengths and ratios). A registered deterministic profile (`derandomize`, bounded examples, no deadline) keeps the required gate reproducible (#718).
- Wire `hypothesis>=6,<7` into the `test` extra, `requirements/constraints.txt` (the envelope mirror is test-enforced), and the `pr_checks_tests.yml` install line, so the property suite runs inside the required `PR Checks Tests` gate; OpenSSF Scorecard's Fuzzing check detects Hypothesis usage, and the Best Practices Gold criterion `dynamic_analysis` is met by the same work (#718).
- Pin the new surfaces in `test_supply_chain_surfaces`: the workflow install line and the test-extra entry red any PR that silently drops the property-testing capability (#718).

## [5.5.1] - 2026-07-15

- Clear the four moderate npm advisories from the docs-site lockfile — `js-yaml` GHSA-h67p-54hq-rp68 and GHSA-mh29-5h37-fv8m, `markdown-it` GHSA-38c4-r59v-3vqw and GHSA-6v5v-wf23-fmfq — by extending the existing `overrides` stanza in `docs-site/package.json` (`js-yaml@^4` → `^4.2.0`; `markdown-it` → `^14.2.0`, the higher of the two advisories' fixed floors) and regenerating `package-lock.json`; the resolved graph now carries js-yaml 3.15.0/4.3.0 and markdown-it 14.3.0, and the full Docusaurus build passes under the lifted versions (#719).
- Pin the advisory floors inside the required `PR Checks Tests` gate: `test_supply_chain_surfaces` parses the site lockfile and reds any PR whose resolved `js-yaml` falls below 3.15.0/4.2.0 by major or whose `markdown-it` falls below 14.2.0, so a future Docusaurus upgrade or lockfile regeneration cannot silently reintroduce them (#719).

## [5.5.2] - 2026-07-16

- Deliver slice-gate verdicts rerun-first: `pr_checks_slice_on_issue.yml` and `pr_checks_slice_sweep.yml` now look up the canonical `pr_checks_slice` pull_request run for each affected PR head and rerun it when its conclusion diverges from the computed verdict (new job-scoped `actions: write`), so the checks UI keeps a single truthful entry that flips green after Done Means evidencing and red on issue drift with zero manual reruns; the API check-run POST remains only as the fail-closed fallback when no rerunnable canonical run exists. Downstream of the stale-red observed live on PR #723 and filed upstream as Vaquum/new-repository-template#65 (#724).
- Add `.github/workflows/pr_merge_readiness.yml`: one auto-updated, marker-tagged comment per PR that evaluates every merge condition together — each required context's latest conclusion on the head, the review decision, and the unresolved-conversation count — with a single verdict line, refreshing on pushes, review events, thread resolution, and check-suite completions (self-retrigger guarded); informational only, never a required check (#724).
- Require the three-gate status triple in agent PR reports via `Claude.md`: required checks n/n, review decision, and unresolved-conversation count from a live same-turn fetch, with "mergeable" reserved for the full conjunction (#724).
- Pin the new surfaces in `test_governance_hardening_surfaces`: the rerun delivery and `actions: write` scopes in both verdict workflows, and the readiness workflow's trigger families and comment permission (#724).

## [5.6.0] - 2026-07-16

- Hash-pin every workflow dependency install: `requirements/ci/` gains nine pip-compile-generated sets (`--generate-hashes`, resolved on Python 3.10 for Linux, compiled from the same envelope as `requirements/constraints.txt`, with the project excluded via `--no-emit-package`), and every `pip install` across the ten pip-installing workflows becomes `pip install --require-hashes -r requirements/ci/<profile>.txt` followed by `pip install --no-deps -e .` (or the built wheel) where the project itself is needed — closing the range-substitution supply-chain hole OpenSSF Scorecard's Pinned-Dependencies check scores. The two `--upgrade pip` invocations are removed outright, and the multi-Python Package Install matrix keeps its constraints-based built-wheel install, the one tolerated unhashed site (#720).
- Update the release policy's workflow-hardening invariant from constraints-bounded installs to hash-verified installs, with `requirements/constraints.txt` remaining the human-readable envelope the sets are compiled from (#720).
- Pin the new bar inside the required `PR Checks Tests` gate: `test_supply_chain_surfaces` reds any PR that adds a workflow `pip install` without `--require-hashes`, drops a hashed set, or removes the policy line (#720).
- At merge time, enable `require_last_push_approval` on branch ruleset 5406995 via the API (recorded in the slice closeout), so a post-approval push requires re-approval — the Branch-Protection lever of PRD #717 (#720).
...
- Clear the two npm advisories that redded the required `PR Checks Docs Site` gate on every branch — `brace-expansion` GHSA-mh99-v99m-4gvg (DoS via unbounded expansion causing an out-of-memory crash) and `postcss` GHSA-r28c-9q8g-f849 (path traversal in source-map auto-loading disclosing arbitrary `.map` files), both high. The existing `brace-expansion` override at `^1.1.16` did not clear the advisory: it covers every version `<= 5.0.7`, so the overrides stanza now pins the first patched release `5.0.8`, and `postcss` gains a `^8.5.18` floor over the resolved `8.5.15`. `npm --prefix docs-site run security:audit` reports no vulnerabilities and `npm --prefix docs-site run check` builds clean.
- Pin `astral-sh/ruff-action` to `0.15.21`, the release the hashed dev environment already installs. The action carried no version input and resolved the newest ruff, so 0.16.0 stabilising `PLR0917` and changing `RUF100`/`BLE001` handling redded the required `PR Checks Ruff` gate on every branch at once against roughly 25 pre-existing violations in files no PR had touched. The gate now executes the version the repo pins, and raising it becomes a deliberate change.

## [5.9.5] - 2026-07-26

- Adopt the canonical governance gate library from `Vaquum/new-repository-template`: fifteen gate modules Limen did not have (`check_budget_ratchet`, `check_coverage_floor`, `check_coverage_ratchet`, `check_diff_coverage`, `check_docstrings`, `check_file_size_balance`, `check_module_budgets`, `check_module_docstrings`, `check_no_swallowed_violations`, `check_test_code_ratio`, `check_test_fallbacks`, `fail_loud_gate`, `typing_gate`, `version_gate`) plus refreshes of the four it had behind. `slice_gate.py` gains the extracted `_*_failures` helpers and 332 lines of upstream hardening; `_common.py` gains `TYPING_BUDGET`, `find_python_files`, `resolve_package_dir`, and `significant_lines`; `check_dependency_vulnerabilities.py` picks up its upstream fixes. Every module is pure stdlib plus `_common`, so no dependency changes: `pyyaml` was already pinned in `requirements/ci/gate-tools.in`. No gate becomes a required check in this slice (#775).
- Drop the four Limen-local divergence notes from `cc_gate.py`: upstream has absorbed and improved all of them. The attribution scan no longer treats `api` or `code` as Gemini qualifiers, so "Gemini API client" — ordinary exchange vocabulary in this repository — stops hard-failing a required gate; the `co-authored-by` alternation is word-bounded so a surname like Hillman cannot match `llm` as a substring; and the linked-issue rule now checks every closing-referenced issue except `planning`-labelled parent PRDs rather than only `slice`-labelled ones, which is the broader form.
- Add the gate test layer as `governance/tests/`: eleven unit-test modules, `conftest.py`, and the ruleset fixtures, 99 tests passing. The thirteen contract-test modules that pin the workflows, the ruleset snapshot, and the constitution are deliberately not in this slice — they assert surfaces that do not exist yet — and land with those surfaces. `ruleset_gate.py` and `privileged_ruleset_audit.py` are held back for the same reason, so every module here ships with its own proof.
- Calibrate three budget files at Limen's measured state rather than at zero, so the ratchet laws hold from the first PR without a preparatory clean-up: `.github/module_budgets.json` declares 413 modules (394 under `limen/`, 19 under `governance/`) each at its current significant-line count rounded up to the next ten; `.github/typing_budget.json` seeds 879 `Any` references across ten patterns with `package_root` set to `limen`; `.github/fail_loud_budget.json` seeds the eleven existing silent-fallback sites. Every one can only ever fall. `.github/coverage_budget.json` is not in this slice: its `branch` floor has never been measured for this repository and seeding it with an invented number would defeat the gate.
- Record the measured baseline the rest of the programme closes against: `check_module_docstrings` 393 violations, `check_test_fallbacks` 164 sites across 37 files, `check_docstrings` 9, `check_test_code_ratio` 0.5957 against a 0.60 floor, `check_file_size_balance` 35.83 against a 16.00 bound, and 541 ruff findings in `limen/` plus 63 in `tests/` under the upstream ruleset.

## [5.10.0] - 2026-07-26

- Adopt the eleven-law constitution from `Vaquum/new-repository-template` as `CLAUDE.md`, replacing the XML-tagged prompt that pointed at `Project.md` and was never a constitution. Nineteen laws here rather than upstream's eleven, because law 17 requires one law per required status check and this repository ships a package: alongside the governance gates it carries packaging verification, a built-wheel install matrix on four interpreters, a coverage floor, a suite runtime budget, and a docs site. `AGENTS.md` drops from 257 lines to a five-line pointer, retiring the WA `task_type` regime and its JSON hard-rules block; `.github/copilot-instructions.md` goes from a two-line stub to the full 91-line reviewer brief (#774).
- Add the nine gate workflows the merge path was missing — `pr_checks_lint`, `pr_checks_typing`, `pr_checks_version`, `pr_checks_fail_loud`, `pr_checks_honesty`, `pr_checks_ruleset`, `audit_main_ruleset` — and fold `pr_checks_ruff`, `pr_checks_pyright` and `pr_checks_supply` into them. Refresh the seven self-healing workflows to their upstream form, closing 318 lines of drift that included the API-posted check-run reconciliation whose absence pinned PR #757's merge button red and cost an empty commit to clear.
- Snapshot branch protection as `.github/rulesets/main.json` and put the laws and the required checks into checked bijection: `pr_checks_honesty` fails when a law names a check the ruleset does not require, or the ruleset requires a check no law names. `audit_main_ruleset` re-checks the live ruleset post-merge with a privileged token, including the `bypass_actors` the PR-time gate cannot observe.
- Land `governance.yml` as the contract anchor with this repository's values, plus `ruleset_gate.py`, `privileged_ruleset_audit.py` and the thirteen contract-test modules held back from #775 because they assert surfaces that did not exist yet. The governance suite is 158 tests, all passing.
- Record five upstream gates as deliberately unwired, each named in law 6 with its reason rather than left as silent drift: `check_module_docstrings` and `check_docstrings` (this repository ignores `D100` by choice — one function per file with filename-matching names makes a module docstring a restatement), `check_test_fallbacks` (the pattern it flags is a correct verbose `pytest.raises`, and it mis-flags `try`/`finally` cleanup), `check_test_code_ratio` (a SLOC proxy on a suite already at 93% coverage), and `check_file_size_balance` (pending a configurable bound upstream). Dead-code detection is absent for the same reason: all 17 vulture findings are parameter names in Protocol stubs and sklearn-shaped signatures, so it reports no defect this codebase has.
- Resolve every pinned action to its exact release tag, verified against the live tag refs with annotated tags dereferenced: `actions/checkout` v3.6.0, `actions/setup-python` v4.9.1, `actions/upload-artifact` v4.6.2, `actions/download-artifact` v4.3.0. Repin `py-cov-action/python-coverage-comment-action` from a SHA that was no published tag — whatever `v3` pointed at when pinned — to the v3.41 release head.
- Put the enforcement surfaces under code-owner review: `.github/CODEOWNERS` gains `governance.yml`, `pyproject.toml` and `requirements/`, which now carry the ruff policy, the tool pins and the gate budgets.
- Tighten the `dev` extra's ruff declaration from `>=0.8,<1` to `>=0.15.21,<0.15.22` so a local `pip install -e ".[dev]"` resolves the version the hash-locked CI set installs. The loose range is how the local and CI ruff drifted apart in the first place.

## [5.10.1] - 2026-09-11

- Restore the `pr_checks_ruleset` gate, red on every branch since 2026-08-29 before the gate ran: the workflow created a fresh venv whose bundled pip 23.0.1 treated the `cyclonedx-python-lib[validation]` extra pulled by `cyclonedx-bom==7.3.0` as unpinned under `--require-hashes` and resolved past the locked 11.11.0. It now installs and runs the contract tests with the runner's interpreter, as the lint, typing, honesty, and packaging gates already do.
- Restore the `PR Checks Docs Site` gate, red on every branch over the same period on 34 npm advisories: raise the `brace-expansion` override to 5.0.9 (5.0.8 sits inside the new advisory range and drove `minimatch` and `serve-handler`), move `js-yaml` to 4.3.2 and 3.15.2, pin `qs` to 6.16.0 under `express`, and take the in-range fixes for the rest, which carries Docusaurus to 3.10.2. Track the two `image-size` advisories (GHSA-w3rx-r6r6-pgpr, GHSA-5p2g-fcmc-qvqq) in `docs-site/scripts/audit-security.mjs` with the date and the release that retires them: every published `image-size` is in range and the newest Docusaurus MDX loader requires it, so no bump can clear them.
- Amend law 10 to say what the gate enforces — no untracked advisory — and to define a tracked advisory as a dated exception with a retirement condition, so the constitution and the audit script agree.

## [6.0.0] - 2026-09-11

- **Breaking:** Price the long-flat entry at the signal bar's close. `long_flat_strategy` now computes one close-to-close return per held bar, `close_t / close_{t-1} - 1`, so the entry fill is the close of the signal bar: the price Nexus acts on and the Praxis replay venue fills at. The separate `price_change / open` entry return is removed, and an execution row whose prior close is missing or zero (the first row under `execution_lag_bars=0`, or the row after a price gap) is non-tradable. Every historical ledger value is re-priced; the shift per entry is `(close_{t+1} / open_{t+1}) × (open_{t+1} - close_t) / close_t`, the gap between the next bar's open and the signal bar's close, a few tenths of a basis point per trade on liquid hourly bars. The strategy signature, `backtest_snapshot` and its validation, the 20-column ledger, the fee and slippage multipliers, `notional_rate`, and `execution_lag_bars` are unchanged (#805).
- State the signal-bar close fill in `docs/Backtest.md` Table 2 and mark `execution_lag_bars=0` as a research diagnostic that prices the entry at the previous close, before the signal exists.
- Re-derive the `dollar_bar_crash_reversal` reference-candidate figures in `docs/Built-In-SFDs.md` from a rerun of the bundled template under the new fill: the executed-trade counts are unchanged at 129, 13, and 14, and the mean net PnL per executed trade moves from 75.7, 116.1, and 127.1 bps to 76.0, 115.7, and 127.4 bps.

## [7.0.0] - 2026-09-16

- **Breaking:** Charge fill fees on notional instead of as return multipliers. `long_flat_strategy` pays `fee_bps` of the entry notional from cash at entry and `fee_bps` of the exit proceeds at exit, the way the Praxis replay and paper venues book the taker fee, while slippage stays a fill-price adjustment. Within a trade, equity is the position minus the entry fee and each bar's net return is the equity return, so the entry fee does not compound with the position. A trade with gross return `g` now nets `(1 + g)(1 − fee)(1 − slip) / (1 + slip) − fee − 1`, which is `g − fee × (2 + g)` at zero slippage, the deployed arithmetic, where the multipliers gave `(1 + g)(1 − fee)² (1 − slip) / (1 + slip) − 1`. Every historical ledger value is re-priced: the per-trade shift is exactly `fee × ((1 + g)(1 − fee)(1 − slip) / (1 + slip) − 1)`, about `fee × (g − fee − 2 × slip)`, so at the default 5 bps costs a trade moves by `0.0005 × g` less 0.0075 bps, inside the `0.0175 bps + 0.001 × |g|` the PRD states, and `cost_bps` on a held bar that is neither the entry nor the exit is no longer exactly zero but the small gap between the position's return and the equity return. The per-segment growth is accumulated in log space, so a window with millions of entries keeps every per-bar net within `1e-13` of its definition, where a running product drifts by up to `8e-3` once it reaches the subnormal range. The strategy signature, `backtest_snapshot` and its validation, the 20-column ledger, `notional_rate`, and `execution_lag_bars` are unchanged (#812).
- State the fee-on-notional contract in `docs/Backtest.md` Table 2 and "Economic inputs".
