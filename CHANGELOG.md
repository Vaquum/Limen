...
- **Breaking:** Charge fill fees on notional instead of as return multipliers. `long_flat_strategy` pays `fee_bps` of the entry notional from cash at entry and `fee_bps` of the exit proceeds at exit, the way the Praxis replay and paper venues book the taker fee, while slippage stays a fill-price adjustment. Within a trade, equity is the position minus the entry fee and each bar's net return is the equity return, so the entry fee does not compound with the position. A trade with gross return `g` now compounds to `(1 + g)(1 − fee)(1 − slip) / (1 + slip) − fee`, the deployed `g − fee × (2 + g)` at zero slippage, where the multipliers gave `(1 + g)(1 − fee)² (1 − slip) / (1 + slip) − 1`. Every historical ledger value is re-priced: the per-trade shift is exactly `fee × ((1 + g)(1 − fee)(1 − slip) / (1 + slip) − 1)`, about `fee × (g − fee − 2 × slip)`, so at the default 5 bps costs a trade moves by `0.0005 × g` less 0.0075 bps, inside the `0.0175 bps + 0.001 × |g|` the PRD states, and `cost_bps` on a continuation bar is no longer exactly zero but the small gap between the position's return and the equity return. The strategy signature, `backtest_snapshot` and its validation, the 20-column ledger, `notional_rate`, and `execution_lag_bars` are unchanged (#812).
