...
def _experiment_confusion_metrics(
    self, x: str, disable_progress_bar: bool = False
) -> pd.DataFrame:
    """
    ...
    for i in tqdm.tqdm(range(len(self.round_params)), disable=disable_progress_bar):
        result_df = self.permutation_confusion_metrics(
            x=x, round_id=i, id_cols=self.round_params[i]
        )
