...
class Log:
    ...
    from loop.log._experiment_backtest_results import (
        _experiment_backtest_results as experiment_backtest_results,
    )
    from loop.log._experiment_confusion_metrics import (
        _experiment_confusion_metrics as experiment_confusion_metrics,
    )
    from loop.log._experiment_parameter_correlation import (
        _experiment_parameter_correlation as experiment_parameter_correlation,
    )
    from loop.log._permutation_confusion_metrics import (
        _permutation_confusion_metrics as permutation_confusion_metrics,
    )
    from loop.log._permutation_prediction_performance import (
        _permutation_prediction_performance as permutation_prediction_performance,
    )
    ...
    def __init__(
        ...
        cols_to_multilabel: Optional[List[str]] = None,
    ) -> None:
        """
        ...
        if cols_to_multilabel is not None:
            for col in cols_to_multilabel:
                self.experiment_log = wrangle.col_to_multilabel(
                    data=self.experiment_log, col=col, extended_colname=True
                )
    ...
    def _get_test_data_with_all_cols(self, round_id: int) -> pl.DataFrame:
        ...
        if missing_datetimes:
            result = result.join(
                pl.DataFrame({"datetime": missing_datetimes}).with_columns(
        ...
        return result.filter(
            pl.col("datetime").is_between(
                first_test_datetime, last_test_datetime, closed="both"
            )
        )
