...
def _breakout_roc(
    ...
    return data.with_columns(
        [
            ...
            .then(
                ((pl.col(next_short_col) - pl.col(short_col)) / pl.col(short_col)) * 100
            )
            .otherwise(0)
...
def breakout_features(
    ...
    df = _breakout_roc(
        df, current_long_col, current_short_col, next_long_col, next_short_col
    )
    ...
    cols = (
        [f"long_t-{i}" for i in range(horizon, horizon + lookback)]
        ...
        + [
            ...
            "short_roll_mean",
            "short_roll_std",
            f"roc_long{roc_suffix}",
            f"roc_short{roc_suffix}",
            target,
        ]
    )
