...
def market_regime(
    df: pl.DataFrame, lookback: int = 48, short_sma: int = 20, long_sma: int = 50
) -> pl.DataFrame:
    """
    ...
    df = df.rename(
        {f"close_sma_{short_sma}": "sma_20", f"close_sma_{long_sma}": "sma_50"}
    )

    df = df.with_columns([pl.col("close").pct_change().alias("returns_temp")])

    df = df.with_columns(
        [
            (
                pl.col("returns_temp").rolling_std(window_size=12)
                / pl.col("returns_temp").rolling_std(window_size=lookback)
        ...
        [
            (
                (
                    ...
                    + (pl.col("volume_regime") > 0.7).cast(pl.Int32)
                )
                / 3.0
