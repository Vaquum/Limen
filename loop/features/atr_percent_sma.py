...
def atr_percent_sma(data: pl.DataFrame, period: int = 14) -> pl.DataFrame:
    ...
    return (
        data.with_columns([pl.col("close").shift(1).alias("prev_close")])
        .with_columns(
            [
        ...
        .with_columns(
            [
                pl.max_horizontal(["high_low", "high_close", "low_close"]).alias(
                    "true_range"
                )
            ]
        )
        ...
        .with_columns([(pl.col("atr") / pl.col("close")).alias("atr_percent_sma")])
        .drop(
            ["prev_close", "high_low", "high_close", "low_close", "true_range", "atr"]
        )
    )
