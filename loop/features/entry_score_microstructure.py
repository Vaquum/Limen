...
def entry_score_microstructure(
    data: pl.DataFrame,
    ...
    entry_spread_ratio_max: float = 2,
) -> pl.DataFrame:
    """
    ...
    df = df.with_columns(
        [
            (
                ...
                + (pl.col("micro_momentum") > 0).cast(pl.Float32)
                * entry_momentum_weight_base
                + pl.col("volume_spike").clip(
                    entry_volume_spike_min, entry_volume_spike_max
                )
                / entry_volume_spike_normalizer
                * entry_volume_weight_base
                + (
                    1
                    - (
                        pl.col("spread_percent")
                        / pl.col("spread_percent").rolling_mean(
                            window_size=spread_mean_period
                        )
                    ).clip(entry_spread_ratio_min, entry_spread_ratio_max)
                )
                * entry_spread_weight_base
    ...
    df = df.with_columns(
        [
            ...
            .then(
                ...
                + (pl.col("micro_momentum") > 0).cast(pl.Float32)
                * entry_momentum_weight_low_vol
                + pl.col("volume_spike").clip(
                    entry_volume_spike_min, entry_volume_spike_max
                )
                / entry_volume_spike_normalizer
                * entry_volume_weight_low_vol
                + (
                    1
                    - (
                        pl.col("spread_percent")
