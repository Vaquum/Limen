...
def vwap(
    data: pl.DataFrame, price_col: str = "close", volume_col: str = "volume"
) -> pl.DataFrame:
    """
    ...
    return (
        data.sort("datetime")
        .with_columns([pl.col("datetime").dt.date().alias("__date")])
        .with_columns(
            [
                ...
                .over("__date")
                .alias("__cum_pv"),
                pl.col(volume_col).cum_sum().over("__date").alias("__cum_vol"),
            ]
        )
        .with_columns([(pl.col("__cum_pv") / pl.col("__cum_vol")).alias("vwap")])
