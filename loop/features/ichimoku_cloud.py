...
def ichimoku_cloud(
    ...
    return (
        data.with_columns(
            (
                (
                    pl.col("high").rolling_max(tenkan_period)
                    + pl.col("low").rolling_min(tenkan_period)
                )
                / 2
            ).alias("tenkan")
        )
        .with_columns(
            (
                (
                    pl.col("high").rolling_max(kijun_period)
                    + pl.col("low").rolling_min(kijun_period)
                )
                / 2
            ).alias("kijun")
        )
        .with_columns(
            ((pl.col("tenkan") + pl.col("kijun")) / 2)
            .shift(-displacement)
            .alias("senkou_a")
        )
        .with_columns(
            (
                (
                    pl.col("high").rolling_max(senkou_b_period)
                    + pl.col("low").rolling_min(senkou_b_period)
                )
                / 2
            )
            .shift(-displacement)
        ...
        .with_columns(pl.col("close").shift(displacement).alias("chikou"))
    )
