...
def exit_quality(
    ...
    return data.with_columns(
        [
            pl.when(
                (pl.col("exit_reason").is_in(["target_hit", "trailing_stop"]))
                & (pl.col("exit_net_return") > 0)
            )
            .then(pl.lit(exit_quality_high))
            .when(
                (pl.col("exit_reason") == "stop_loss")
                | (
                    (pl.col("exit_reason") == "timeout")
                    & (pl.col("exit_net_return") < 0)
                )
            )
