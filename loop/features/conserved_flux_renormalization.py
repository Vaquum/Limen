...
def conserved_flux_renormalization(
    ...
    *,
    ...
    trades = trades_df.with_columns(
        (pl.col("price") * pl.col("quantity")).alias("value")
    ).sort("datetime")

    kl = (
        trades.group_by_dynamic("datetime", every=kline_interval, closed="left")
        .agg(
    ...
    feats = trades.group_by_dynamic(
        "datetime", every=kline_interval, closed="left"
    ).map_groups(_bar_features, schema=feature_schema)

    return kl.join(feats, on="datetime", how="left").sort("datetime")
