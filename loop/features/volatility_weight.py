...
def volatility_weight(
    ...
    volatility_weight_max: float = 1.0,
) -> pl.DataFrame:
    """
    ...
    df = data.with_columns([pl.col("close").pct_change().alias("returns_temp")])
    ...
    df = df.with_columns(
        [
            (2 / (1 + pl.col("volatility") * volatility_scaling_factor))
            .clip(volatility_weight_min, volatility_weight_max)
            .alias("volatility_weight")
        ]
    )
