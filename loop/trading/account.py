...
class Account:
    ...
    def update_account(self, action, amount, price_usdt):
        ...
        if not (amount > 0 or action == "hold"):
            raise ValueError(
                "ERROR: amount must be positive for all actions except hold."
            )
        if price_usdt <= 0:
        ...
        current_long_btc = (
            self._cached_long_position
        )  # PERFORMANCE FIX: use cached value
        ...
        if action == "buy":
            ...
            debit_usdt = amount
            amount_bought_btc = round(
                debit_usdt / price_usdt, 15
            )  # PRECISION FIX: 15 decimals instead of 7
            ...
            amount_borrowed_btc = round(
                amount / price_usdt, 15
            )  # PRECISION FIX: 15 decimals instead of 7
            credit_usdt = amount
            sell_price_usdt = price_usdt
        ...
        calculated_long = sum(self.account["amount_bought_btc"]) - sum(
            self.account["amount_sold_btc"]
        )
        calculated_short = sum(self.account["amount_borrowed_btc"]) - sum(
            self.account["amount_covered_btc"]
        )
        calculated_usdt = sum(self.account["credit_usdt"]) - sum(
            self.account["debit_usdt"]
        )
    ...
    def net_position(self):
        ...
        return (
            self._cached_long_position - self._cached_short_position
        )  # PERFORMANCE FIX: O(1) instead of O(n)
