...
def confusion_matrix_plus(
    df_results: object,
    x: str,
    y_lim_correction: float = 6.5,
    outlier_quantiles: list = [0.01, 0.99],
) -> None:
    """
    ...
    conditions = [
        (
            (df_filtered["predictions"] == 1) & (df_filtered["actuals"] == 1),
            "True Positive",
            "#9D8CD8",
        ),
        (
            (df_filtered["predictions"] == 1) & (df_filtered["actuals"] == 0),
            "False Positive",
            "#FFB3D9",
        ),
        (
            (df_filtered["predictions"] == 0) & (df_filtered["actuals"] == 0),
            "True Negative",
            "#9D8CD8",
        ),
        (
            (df_filtered["predictions"] == 0) & (df_filtered["actuals"] == 1),
            "False Negative",
            "#FFB3D9",
        ),
    ]
    ...
    fig.suptitle(
        f"Model Score: ${model_score:.0f}", fontsize=18, fontweight="bold", y=0.98
    )
