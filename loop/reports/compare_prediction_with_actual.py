...
def compare_prediction_with_actual(
    prep: callable, model: object, validate: object
) -> None:
    """
    ...
    axes[0, 1].set_title(
        "Do distributions of predictions closely mirror actuals?", fontsize=12
    )

    directional_accuracy = np.sign(df["actuals"]) == np.sign(df["predictions"])
    directional_accuracy.value_counts().plot.bar(
        ax=axes[0, 2], alpha=0.6, color=["green", "red"]
    )
    ...
    axes[1, 2].set_title(
        "Are predictions closely clustered around the identity line?", fontsize=12
    )
