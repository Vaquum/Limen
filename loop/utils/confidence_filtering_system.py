...
def calibrate_confidence_threshold(models, x_val, y_val, target_confidence=0.8):
    ...
    if np.sum(confident_mask) > 0:
        confident_mae = mean_absolute_error(
            y_val[confident_mask], val_pred_mean[confident_mask]
        )
    ...
    if np.sum(uncertain_mask) > 0:
        uncertain_mae = mean_absolute_error(
            y_val[uncertain_mask], val_pred_mean[uncertain_mask]
        )
...
def apply_confidence_filtering(models, x_test, y_test, confidence_threshold):
    ...
    if np.sum(confident_mask) > 0:
        confident_mae = mean_absolute_error(
            y_test[confident_mask], test_pred_mean[confident_mask]
        )
    ...
    if np.sum(uncertain_mask) > 0:
        uncertain_mae = mean_absolute_error(
            y_test[uncertain_mask], test_pred_mean[uncertain_mask]
        )
...
def confidence_filtering_system(
    models: list, data: dict, target_confidence: float = 0.8
) -> tuple:
    """
