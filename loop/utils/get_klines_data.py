...
def get_klines_data(n_rows: Optional[int] = None,
    ...
    query = (
        ...
        f"stats AS ( "
        f"  SELECT "
        f"    avg(close) AS mean_price_val, "
        f"    stddevPop(close) AS std_price_val, "
        f"    quantile(0.5)(close) AS median_price_val, "
        f"    quantile(0.75)(close) - quantile(0.25)(close) AS iqr_price_val "
        f"  FROM klines "
        f") "
        f"SELECT "
        f"  klines.*, "
        f"  stats.mean_price_val AS mean_price, "
        f"  stats.std_price_val AS std_price, "
        f"  stats.median_price_val AS median_price, "
        f"  stats.iqr_price_val AS iqr_price "
        f"FROM klines "
        f"CROSS JOIN stats"
    )
