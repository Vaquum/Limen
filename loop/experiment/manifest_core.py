...
class FittedTransformBuilder:
    ...
    def fit_param(
        self, name: str, compute_func: Callable, **params
...
class Manifest:
    ...
    def set_data_source(
        self, method: Callable, params: Dict[str, Any] = None
    ) -> "Manifest":
        """
        ...
        self.data_source_config = DataSourceConfig(method=method, params=params or {})

        return self

    def set_test_data_source(
        self, method: Callable, params: Dict[str, Any] = None
    ) -> "Manifest":
        """
    ...
    def compute_test_bars(
        self, raw_data: pl.DataFrame, round_params: Dict[str, Any]
    ) -> pl.DataFrame:
        """
    ...
    def prepare_data(
        ...
        datetime_bar_pairs = [
            _process_bars(self, split, round_params) for split in split_data
        ]
        ...
        return _finalize_to_data_dict(
            self, split_data, all_datetimes, all_fitted_params, round_params
        )
    ...
    def run_model(self, data: dict, round_params: Dict[str, Any]) -> dict:
        ...
        if self.model_function is None:
            raise ValueError(
                "Model function not configured. Use .with_model(model_function) before run_model()."
            )
...
def _resolve_params(
    params: Dict[str, Any], round_params: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def _process_bars(
    manifest: Manifest, data: pl.DataFrame, round_params: Dict[str, Any]
...
def _apply_feature_transforms(
    manifest: Manifest, lazy_data, round_params: Dict[str, Any]
):
...
def _apply_target_transforms(
    ...
    return _apply_fitted_transforms(
        manifest.target_transforms,
        data,
        enhanced_round_params,
        all_fitted_params,
        is_training,
    )
...
def _apply_scaler(
    ...
    if manifest.scaler:
        return _apply_fitted_transforms(
            [manifest.scaler], data, round_params, all_fitted_params, is_training
        )
    ...
    if manifest.target_column:
        for i, split_df in enumerate(split_data):
            ...
            if manifest.target_column in cols:
                ...
                raise ValueError(
                    f"Split {i} missing target column '{manifest.target_column}'"
                )
