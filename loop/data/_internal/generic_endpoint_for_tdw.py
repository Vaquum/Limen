...
def generic_endpoint_for_tdw(
    ...
    if month_year is not None and n_rows is None:
        ...
        raise ValueError(
            "Exactly one of month_year or n_rows must be provided, not both or neither."
        )

    query = f"SELECT {', '.join(select_cols)} FROM tdw.{table_name} {where}"
