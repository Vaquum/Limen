...
def _standard_bars(
    data: pl.DataFrame, threshold: Union[int, float], column_name: str
) -> pl.DataFrame:
    """
    ...
    last_bar_sum = df_with_groups.filter(pl.col("bar_group") == max_bar_group)[
        column_name
    ].sum()

    if last_bar_sum < threshold:
        df_with_groups = df_with_groups.filter(pl.col("bar_group") < max_bar_group)

    result = (
        df_with_groups.drop(["index", "threshold_col"])
...
def liquidity_bars(data: pl.DataFrame, liquidity_threshold: float) -> pl.DataFrame:
    """
