...
def sma_deviation_std(
    data: pl.DataFrame, period: int = 30, window: int = 30
) -> pl.DataFrame:
    """
    ...
    return (
        ...
        .with_columns([(pl.col("close") - pl.col(sma_col)).alias(dev_col)])
        .with_columns([pl.col(dev_col).rolling_std(window_size=window).alias(out_col)])
