...
def render_pivot_table(
    ...
    *,
    ...
    if (
        pivot_rows
        and quantile_rows
        and pd.api.types.is_numeric_dtype(work_df[pivot_rows])
    ):
        edges, labels = quantile_bins_fn(work_df[pivot_rows])
        qname = pivot_rows + "_q"
        work_df[qname] = pd.cut(
            work_df[pivot_rows], bins=edges, labels=labels, include_lowest=True
        )
        idx = [qname]
    ...
    if (
        pivot_cols
        and quantile_cols
        and pd.api.types.is_numeric_dtype(work_df[pivot_cols])
    ):
        edges, labels = quantile_bins_fn(work_df[pivot_cols])
        qname = pivot_cols + "_q"
        work_df[qname] = pd.cut(
            work_df[pivot_cols], bins=edges, labels=labels, include_lowest=True
        )
        cols = [qname]
    ...
    title = (
        "Pivot table rendered as heatmap"
