...
def render_trend_controls(df: pd.DataFrame) -> dict:
    ...
    with st.sidebar:
        ...
        period = st.number_input(
            "Period", min_value=2, max_value=1000, value=24, step=1
        )
        ...
        if method == "MA Slope":
            params["threshold"] = st.number_input(
                "Slope Threshold (normalized)", value=0.0, step=0.1, format="%0.1f"
            )
            ...
            params["k"] = st.number_input(
                "k (band multiplier)", value=0.75, step=0.05, format="%0.2f"
            )
        elif method == "Breakout Percentile":
            params["p_hi"] = st.slider(
                "Upper Percentile", 0.5, 0.99, 0.85, 0.01, format="%0.2f"
            )
            params["p_lo"] = st.slider(
                "Lower Percentile", 0.01, 0.5, 0.15, 0.01, format="%0.2f"
            )
        elif method == "Window Return":
            params["r_hi"] = st.number_input(
                "Upper Return Threshold", value=0.0, step=0.001, format="%0.3f"
            )
            params["r_lo"] = st.number_input(
                "Lower Return Threshold", value=0.0, step=0.001, format="%0.3f"
            )
        elif method == "HH/HL Structure":
            params["score_threshold"] = st.number_input(
                "Score Threshold", value=4, step=1
            )
...
def apply_trend_regime(
    ...
    if method == "MA Slope":
        pl_out = ma_slope_regime(
            pl_df,
            period=period,
            threshold=float(settings.get("threshold", 0.0)),
            normalize_by_std=bool(settings.get("normalize_by_std", True)),
        )
        regime_col = "regime_ma_slope"
    elif method == "Price vs Band":
        pl_out = price_vs_band_regime(
            pl_df,
            period=period,
            band=str(settings.get("band", "std")),
            k=float(settings.get("k", 0.75)),
        )
        regime_col = "regime_price_band"
    elif method == "Breakout Percentile":
        pl_out = breakout_percentile_regime(
            pl_df,
            period=period,
            p_hi=float(settings.get("p_hi", 0.85)),
            p_lo=float(settings.get("p_lo", 0.15)),
        )
        regime_col = "regime_breakout_pct"
    elif method == "Window Return":
        pl_out = window_return_regime(
            pl_df,
            period=period,
            r_hi=float(settings.get("r_hi", 0.0)),
            r_lo=float(settings.get("r_lo", 0.0)),
        )
        regime_col = "regime_window_return"
    else:
        pl_out = hh_hl_structure_regime(
            pl_df,
            window=period,
            score_threshold=int(settings.get("score_threshold", 4)),
        )
