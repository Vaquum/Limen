...
from loop.explorer.streamlit_utils import (
    pretty_label,
    format_value,
    quantile_bins_fixed,
)
from loop.explorer.streamlit_outliers import (
    render_outlier_controls,
    apply_outlier_transform,
)
...
def _spacer(rem: float = 5.0) -> None:
    try:
        ...
        rem_val = 5.0
    st.markdown(
        f'<div class="section-spacer" style="height:{rem_val}rem"></div>',
...
if st.session_state["custom_cols"]:
    for _name, _expr in st.session_state["custom_cols"]:
        try:
            df_base[_name] = pd.eval(
                _expr, engine="python", local_dict=df_base.to_dict("series")
            )
        except Exception:
            pass
...
if show_corr:
    render_corr_heatmap(
        df_filt, heatmap_selected_cols if heatmap_selected_cols else num_cols
    )
