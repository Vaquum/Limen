...
# -----------------------------
def _rolling_mean_per_series(
    df_long: pd.DataFrame, series_col: str, value_col: str, window: int
) -> pd.Series:
    """
    ...
    return df_long.groupby(series_col)[value_col].transform(
    ...
    *,
    ...
    if normalize_line:
        ...
        plot_long = df_filt[[xcol] + ycols].melt(
            id_vars=[xcol], value_vars=ycols, var_name="Series", value_name="Value"
        )
        plot_long["Value"] = pd.to_numeric(plot_long["Value"], errors="coerce")

        # Per-series min/max
        stats = (
            plot_long.groupby("Series")["Value"]
        ...
        if smoothing_window > 1:
            for col in ycols:
                plot_df[col] = (
                    plot_df[col].rolling(window=smoothing_window, min_periods=1).mean()
                )
...
def plot_area(
    ...
    *,
    ...
    plot_long = df_filt[[xcol] + ycols_sorted].melt(
        id_vars=[xcol], value_vars=ycols_sorted, var_name="Series", value_name="Value"
    )
    plot_long["Value"] = pd.to_numeric(plot_long["Value"], errors="coerce")

    # Sort + optional smoothing
    plot_long = plot_long.sort_values(["Series", xcol])
    plot_long["Value"] = _rolling_mean_per_series(
        plot_long, "Series", "Value", smoothing_window
    )
    ...
    *,
    ...
    if ycols:
        ...
        long_df = (
            df_filt[cols]
            .copy()
            .melt(value_vars=cols, var_name="Series", value_name="Value")
        )
        if normalize_data:
            ...
            long_df["Value"] = long_df.groupby("Series")["Value"].transform(
                minmax_scale
            )
        ...
        _apply_plot_theme(fig, title=title)
        key = (
            f"hist_multi_{'_'.join(cols)}_{int(normalize_counts)}_{int(normalize_data)}"
        )
    ...
    st.plotly_chart(
        ...
        key=f"hist_single_{ycol}_{int(normalize_counts)}_{int(normalize_data)}",
    )
