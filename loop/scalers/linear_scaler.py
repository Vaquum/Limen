    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        for col in df.columns:
            ...
            if rule == "standard":
                ...
                exprs.append(
                    ((pl.col(col).log1p() - self.means[col]) / self.stds[col]).alias(
                        col
                    )
                )
...
def inverse_transform(df: pl.DataFrame, scaler: LinearScaler) -> pl.DataFrame:
    ...
    for col in df.columns:
        ...
        if rule == "standard":
            exprs.append(
                (pl.col(col) * scaler.stds[col] + scaler.means[col]).alias(col)
            )

        elif rule == "log_standard":
            exprs.append(
                ((pl.col(col) * scaler.stds[col] + scaler.means[col]).exp() - 1).alias(
                    col
                )
            )
